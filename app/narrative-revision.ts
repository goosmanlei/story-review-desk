...
export function runtimeTotal(scenes: NarrativeScene[]): Pick<RuntimeEstimate, 'compactSec' | 'baseSec' | 'spaciousSec'> {
  return scenes.reduce((sum, scene) => ({ compactSec: sum.compactSec + scene.runtime.compactSec, baseSec: sum.baseSec + scene.runtime.baseSec, spaciousSec: sum.spaciousSec + scene.runtime.spaciousSec }), { compactSec: 0, baseSec: 0, spaciousSec: 0 });
}
export function runtimeLabel(seconds: number) { return `${Math.floor(seconds / 60)}分${String(Math.round(seconds % 60)).padStart(2, '0')}秒`; }
