...
type Plan={kind:string;candidate?:Candidate;review?:{eventId:string;action:string};state?:{canFlowDownstream?:boolean};canAuthor:boolean;blockedReason:string;basisBindings:unknown[];template?:{planId:string;revisionHash:string}};
...
export function EpisodeProductionEntry({episodeUid,sceneId}:{episodeUid:string;sceneId:string}) {
  ...
  return <section className="episode-production-entry" aria-label="本集正式镜头拆解">
    <header><h3>本场正式镜头拆解</h3><p>本集正文生效后即可逐场推进；其他集未完成，不阻断本场。</p></header>
    ...
    {entry?.plans.map(plan=><ScenePlanningReview key={`${sceneId}:${plan.kind}:${plan.candidate?.creativeRevisionId||'none'}`} plan={plan} sceneId={sceneId} snapshotId={entry.snapshotId} readOnly={hostedReadOnly} onSaved={()=>setAttempt(v=>v+1)}/>)}
    {entry&&<p>本场正式镜头分母：{entry.formalShotCount===null?'待确定（UNKNOWN）':`${entry.formalShotCount} 镜`}。本集放行不等于素材、镜头输入或成片已通过。</p>}
  </section>;
}

function ScenePlanningReview({plan,sceneId,snapshotId,readOnly,onSaved}:{plan:Plan;sceneId:string;snapshotId:string;readOnly:boolean;onSaved:()=>void}) {
