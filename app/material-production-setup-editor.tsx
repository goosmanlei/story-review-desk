...
function Editor({requirementId,revision}:{requirementId:string;revision:boolean}){
 ...
 const revising=['REVISION','LEGACY_AUDIO_REVISION'].includes(state?.mode||'')||revision;
 ...
 async function act(action:'save'|'preview'|'publish'){
  if(!state||state.readOnly||busy||uncertain||state.mode==='LEGACY_AUDIO_REVISION'&&state.blockers.length)return;setBusy(true);setMessage('');let submitted=false;
  try{
