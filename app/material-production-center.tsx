...
function MaterialCandidateProductionFacts({
  ...
  useEffect(() => {
    ...
    void fetch(`/api/v8/asset-versions?${query.toString()}`, { cache: 'no-store', signal: controller.signal })
      .then(async (response) => {
...
function AssetReviewForm({
  ...
  const reviewEligible = Boolean(version && (lifecycleState === 'REVIEW_PENDING' || (decisionApplied && correctionState === 'OPEN')));
  ...
  async function requestAIReview() {
    ...
    try {
      ...
      const response = await fetch('/api/v8/material-review-drafts', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Idempotency-Key': `material-ai-review-${key}`,
          'If-Match': operations.mutationEtag,
        },
        body: JSON.stringify(body),
      });
      const payload = await response.json() as { draft?: MaterialAIReviewDraft; error?: string; requestId?: string; evidenceBoundary?: string };
      if (!response.ok || !payload.draft) throw new Error(`${payload.error || `HTTP ${response.status}`}${payload.requestId ? `（请求 ${payload.requestId}）` : ''}`);
      ...
      {!familySelectionMismatch && !versionSelectionMismatch && !deletedVersionSelection && <>
        ...
        <section className="material-production-materials" data-material-section="production" data-production-version-id={selectedVersion?.id || explicitSelectedExpected?.id || 'NO_VERSION'}>
          ...
          {selectedFamily
            ...
            : <p className="v6-empty-note">尚未建立可绑定的资产族；生产资料仍为待补齐。</p>}
        </section>
