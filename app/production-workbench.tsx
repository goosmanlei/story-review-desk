...
export type OperationalStateProjection = {
  ...
  assetFamiliesById: Record<string, EntityStateProjection & { currentVersionId?: string | null; adoptedVersionId?: string | null; decisionVersionId?: string | null; currentExpectedOutputId?: string | null; realizedExpectedOutputIds?: string[]; versionRefs?: string[] }>;
...
function ScopeStageContext({ model, shot, workPackage, item, task, reviewContext }: { model: ProductionModel; shot: V7Shot; workPackage: V7WorkPackage; item: V7WorkItem; task: PostProductionTask | null; reviewContext: ScopedReviewContext | null }) {
  ...
  return <section className="creator-scope-stage-context">
    ...
    {workPackage.stepId === 'W09' && <div className="creator-project-gates"><article><b>故事与案件</b><p>核对人物关系、死亡与割首顺序、九颗首级和尸身位置、物证链及审案结论。</p></article><article><b>版本与禁用</b><p>经审阅锁定的分集采用版本、字幕、分轨、哈希与归档必须齐套，任何禁用资产不得下传；{currentEpisodePlan ? `当前${model.episodes.length}集方案已采用，但尚未建立正式制作分母。` : `当前${model.episodes.length}段仅为导航提案。`}</p></article><article><b>发行门禁</b><p>平台、受众／分级、发行编码／色彩／响度、商业权利与法律放行仍为UNKNOWN。</p></article></div>}
