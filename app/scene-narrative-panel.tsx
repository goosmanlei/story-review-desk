...
const stateLabels: Record<SceneNarrativeContext['upstreamState'],string> = {PENDING_REVIEW:'待审要求',REVISION_REQUIRED:'要求修改',DO_NOT_USE:'禁止使用',APPROVED_PENDING_SYNC:'已获批 · 待同步',CURRENT:'已生效要求'};
export function useSceneNarrativeContext(plan: EpisodePlanContext,sceneId: string) {
  ...
  useEffect(()=>{
    ...
    const load=async()=>{try{
      const response=await fetch(`/api/v8/ui/scene-review-context?revisionId=${encodeURIComponent(plan.revisionId)}&sceneId=${encodeURIComponent(sceneId)}`,{cache:'no-store',signal:controller.signal});
