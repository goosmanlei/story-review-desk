...
export async function GET(request:Request) {
 try {
  ...
  return await committedWorkspaceGet(repo,`episode-plan:${new URL(request.url).search}`,async()=>{const response=await readResponse(request);const value=await response.json() as {error?:string;message?:string};if(!response.ok)throw new HttpError(response.status,value.error||value.message||"读取失败");return value;},request);
