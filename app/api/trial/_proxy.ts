  ...
  try {
    ...
    if (instanceRepositoryMode()) {
      ...
      if (request.method === 'GET' && endpoint === '/api/trial/scopes') {
        if(request.headers.get('X-Review-Workspace')==='1') return await committedWorkspaceGet(repository,'trial:scopes',()=>instanceTrialScopes(repository),request);
      ...
      if (request.method === 'GET' && endpoint === '/api/trial/snapshot') {
        ...
        if(request.headers.get('X-Review-Workspace')==='1') return await committedWorkspaceGet(repository,`trial:snapshot:${explicitScope||''}`,()=>instanceTrialSnapshot(repository,{scopeId:explicitScope}),request);
