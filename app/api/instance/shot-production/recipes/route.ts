...
export async function GET(request:Request){try{
 ...
 const repo=await domainRepository();return repo.readTransaction(async tx=>jsonResponse({...await getShotRecipeWorkspace(tx,{workItemId,api}),readOnly:store.instanceReadOnlyMode()}));
...
export async function POST(request:Request){try{
 ...
 if(action==='preview')return repo.readTransaction(async tx=>jsonResponse(await previewShotRecipe(tx,{workItemId,draftRevisionId:requiredString(body,'draftRevisionId')},{api})));
