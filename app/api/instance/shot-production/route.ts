...
export async function GET(request:Request){try{
 ...
 const repo=await domainRepository();return await committedWorkspaceGet(repo,`shot-production:${sceneId}:${store.instanceReadOnlyMode()}`,async tx=>({...await getShotProductionWorkspace(tx,{sceneId,api}),readOnly:store.instanceReadOnlyMode()}),request);
