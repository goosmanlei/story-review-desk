...
export async function GET(request:Request){try{
 ...
 const repo=await domainRepository();return repo.readTransaction(async tx=>jsonResponse({...await getShotProductionWorkspace(tx,{sceneId,api}),readOnly:store.instanceReadOnlyMode()}));
...
export async function POST(request:Request){try{
 ...
 if(action==='preview')return repo.readTransaction(async tx=>jsonResponse(await previewShotProduction(tx,{sceneId,draftRevisionId:requiredString(body,'draftRevisionId')},{api})));
 if(action==='manifest-preview')return repo.readTransaction(async tx=>jsonResponse(await previewShotProductionManifest(tx,{workItemId:requiredString(body,'workItemId'),api})));
