...
export async function POST(request:Request){try{
 const {idempotencyKey,ifMatch}=await domainMutation(request),body=domainBody(await request.json(),['action','requirementId','expectedReleaseId','expectedDraftRevisionId','content','draftRevisionId','previewHash','requestId']);
 ...
 return await repo.writeTransaction(async tx=>{
  ...
  if(action==='save')return jsonResponse(await saveMaterialProductionDraft(tx,{requirementId,expectedReleaseId:requiredString(body,'expectedReleaseId'),expectedDraftRevisionId:expectedRevision(body),content:body.content},{api}));
