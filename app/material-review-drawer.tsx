...
export function MaterialReviewDrawer({open,title,kind,onClose,children}:{open:boolean;title:string;kind:string;onClose:()=>boolean;children:ReactNode}){
 ...
 return <dialog ref={dialog} className="material-review-drawer" data-material-panel={kind} aria-label={title} onCancel={event=>{event.preventDefault();onClose();}} onClick={event=>{if(event.target!==event.currentTarget)return;const rect=event.currentTarget.getBoundingClientRect();if(event.clientX<rect.left||event.clientX>rect.right||event.clientY<rect.top||event.clientY>rect.bottom)onClose();}}>
