...
type Message = { mode?:'DISCUSS'|'EXECUTE';executionReceipts?:ExecutionReceipt[];id: string; role: string; text: string; status: string; context?: ContextView; contextUnavailable?: boolean; workContext?: WorkContextResult; unknowns?: string[]; evidence?: { path: string; note: string }[] };
...
export function CodexConversationDock() {
  ...
  const [allowSettingsPublish,setAllowSettingsPublish]=useState(false);
  const [conversation, setConversation] = useState<Conversation | null>(null);
  const [conversationId, storeConversationId] = useState('');
  const setConversationId = useCallback((value:string) => {storeConversationId(value);setMode('DISCUSS');setAllowSettingsPublish(false);},[]);
  ...
  async function send() {
    ...
    const clearSentComposer = () => {setComposer((current) => canonicalJson(current) === sentComposerSignature ? EMPTY : current);setMode('DISCUSS');setAllowSettingsPublish(false);};
    const frozen = { ...composer, focus: preview.focus };
    const body = { mode,allowSettingsPublish:mode==='EXECUTE'&&allowSettingsPublish,action: conversation ? 'SEND' : 'START', ...(conversation ? { conversationId: conversation.id, expectedTurnHeadHash: conversation.headHash } : {}), userMessage: frozen.text.trim(), focus: preview.focus, draftTargets: preview.draftTargets, expectedDependencyHash: preview.dependencyHash };
  ...
  async function conversationAction(action: string, target = conversation) {
    ...
    setError('');
    try {
      await jsonRequest(`${API}/conversations`, { action, conversationId: target.id, ...(action === 'CANCEL' ? { turnId: target.activeTurnId } : {}) }, `assistant-${crypto.randomUUID()}`);
  ...
  const canSend = Boolean((mode==='DISCUSS'||bridge?.executionProtocol==='REVIEW_CONTROLLED_ACTIONS_V1')&&preview && composer.text.trim() && validUnicode(composer.text) && !busy && !loading && restoredKey === storageKey && bridgeReady && (!conversationId || conversation) && (!conversation || (conversation.assistantProtocol && conversation.canSend)));
    ...
    {!disabled&&open && <>
      ...
      <aside ref={panel} className={styles.drawer} aria-label="项目 Codex 工作助手" onKeyDown={keyboard} style={{ width: `min(${width}px, 100vw)` }}>
        ...
        {hostedReadOnly ? <div className={styles.hostedNotice}><p>远端镜像只读。请在本地审阅台使用工作助手。</p><a href={runtimePath("http://localhost:3000")}>打开本地审阅台</a></div> : <>
          ...
          <section className={styles.composer}><div className={styles.modeSelector} role="group" aria-label="本轮 Codex 模式"><button type="button" aria-pressed={mode==='DISCUSS'} disabled={busy||Boolean(conversation?.activeTurnId)} onClick={()=>{setMode('DISCUSS');setAllowSettingsPublish(false);}}>讨论</button><button type="button" aria-pressed={mode==='EXECUTE'} disabled={busy||Boolean(conversation?.activeTurnId)||bridge?.executionProtocol!=='REVIEW_CONTROLLED_ACTIONS_V1'} onClick={()=>setMode('EXECUTE')}>执行</button></div>{mode==='EXECUTE'?<div className={styles.executionScope}><small>仅本轮允许保存／预览故事设定草稿、逐场准备稿及用途重核；不执行媒体生成、正式素材裁决、删除或任意命令。</small><label><input type="checkbox" checked={allowSettingsPublish} disabled={busy} onChange={event=>setAllowSettingsPublish(event.target.checked)}/>本轮也允许预览后确认发布故事设定</label></div>:<small>只讨论和提供建议，不更改项目数据。</small>}<label htmlFor={`${id}-input`}>{mode==='EXECUTE'?'本轮需要完成哪些受控修改？':'想结合当前工作讨论什么？'}</label><textarea id={`${id}-input`} ref={input} value={composer.text} maxLength={12000} rows={3} placeholder="例如：这处因果是否成立？请结合前后场给出建议。" onChange={(event) => { if (!event.target.value) setComposer((old) => ({ ...old, text: '', focus: null, draft: null })); else freeze(event.target.value); }} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); if (canSend) void send(); } }} /><div className={styles.toolbar}><small>Enter发送 · Shift+Enter换行</small><button disabled={!canSend} onClick={() => void send()}>{busy ? '发送中…' : '发送'}</button></div><small>{mode==='EXECUTE'?'本轮权限不延续到下一轮；真实执行结果以主机操作回执为准。':'建议采用后仍需在原页面提交。'}</small></section>
          <footer className={styles.footer}><details><summary>连接与显示</summary><p>{bridge?.online ? `本机Codex已连接${bridge.model ? ` · ${bridge.model}` : ''}` : '请在项目中启动 Codex Bridge。'}</p><label>助手宽度<input type="range" min="360" max="620" step="20" value={width} onChange={(event) => setWidth(Number(event.target.value))} /></label></details>{conversation && <button disabled={Boolean(conversation.activeTurnId)} onClick={() => void conversationAction(conversation.archived ? 'RESTORE' : 'ARCHIVE')}>{conversation.archived ? '恢复对话' : '归档对话'}</button>}</footer>
        </>}
      </aside>
    </>}
  </>;
