...
export function useSceneScriptComments(sceneId: string | null, snapshotId: string) {
  ...
  const refresh = useCallback(() => {
    ...
    fetch(`/api/v8/script-comments?sceneId=${encodeURIComponent(sceneId)}&limit=500`, { cache: 'no-store', signal: controller.signal })
      .then(async (response) => {
...
export function SceneScriptCommentsPanel({
  ...
  async function post(body: Record<string, unknown>) {
    ...
    const response = await fetch('/api/v8/script-comments', {
      method: 'POST',
  ...
  async function polishComment() {
    ...
    try {
      ...
      const response = await fetch('/api/v8/script-comments/polish', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'If-Match': etag,
          'Idempotency-Key': `comment-polish-${key}`,
        },
        body: JSON.stringify(body),
      ...
      const payload = await response.json().catch(() => ({})) as {
        ...
        requestMode?: 'POLISH_DRAFT' | 'SUGGEST_FROM_CONTEXT';
        error?: string;
        sourceContext?: { directBeatCount?: number; relatedBeatCount?: number };
      };
      if (!response.ok) {
        const requestLabel = payload.requestId ? `（请求编号 ${payload.requestId}）` : '';
        throw new Error(`${payload.error || `HTTP ${response.status}`}${requestLabel}`);
      }
  ...
  return <section id={`scene-script-comments-${target.sceneId}`} className="scene-script-comments" aria-labelledby={`scene-script-comments-title-${target.sceneId}`}>
    ...
    {!loading&&!error&&<PaginatedClosedCommentHistory<ScriptCommentThread> key={target.sceneId} endpoint={`/api/v8/script-comments?sceneId=${encodeURIComponent(target.sceneId)}`} onThreadRead={onReadClosedComment} renderActions={thread=><button type="button" disabled={!thread.anchorMatchesCurrentText} onClick={()=>focusAnchor(thread)}>定位原圈选</button>}/>}
