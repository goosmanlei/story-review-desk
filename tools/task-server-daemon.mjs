...
await save(record);
let logBytes=0;const log=await open(spec.log,'a',0o600);
...
child.once('close',async(code,signal)=>{record={...record,status:'STOPPED',exitCode:code,signal,stoppedAt:new Date().toISOString()};await save(record);await log.close();process.exit(code||0);});
