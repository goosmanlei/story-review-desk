...
async function browserFixture(page:Page){
 if(!browserCode){
  ...
  browserCode=buildSync({stdin:{contents:code,resolveDir:site,sourcefile:'narrative-reading-harness.tsx',loader:'tsx'},bundle:true,platform:'browser',format:'iife',jsx:'automatic',write:false,logLevel:'silent',define:{'process.env.NODE_ENV':'"production"','process.env.NEXT_PUBLIC_REVIEW_BASE_PATH':'""'}}).outputFiles[0].text;
 }
