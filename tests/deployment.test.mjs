...
test("deployment targets and outcomes are explicit", () => {
  ...
  for (const target of ["local", "vps-bj", "all"])
    assert.equal(parseDeploymentArgs(["--target", target]).target, target);
  ...
  assert.equal(
    deploymentExit({
      cleanup: "CLEANED",
      targets: { local: { status: "RESULT_UNKNOWN" } },
    }),
    1,
  );
  assert.equal(
    deploymentExit({
      cleanup: "CLEANED",
      targets: { local: { status: "SUCCEEDED" } },
    }),
    0,
  );
  assert.equal(
    checkRemoteConnection("unreachable", {
      execute: () => {
...
test("clean Git snapshot stays fixed; owned software swaps recover and protect user changes", async (t) => {
  ...
  await writeFile(
    ...
    '{"name":"story-review-desk"}',
  );
  await writeFile(path.join(source, "server/schema.sql"), "-- fixture");
  await writeFile(path.join(source, "tools/deploy.mjs"), "// fixture");
  await writeFile(path.join(source, "file.txt"), "one");
  git("add", ".");
  git("commit", "-m", "fixture one");
  const selected = await resolveSource(source);
  await writeFile(path.join(source, "file.txt"), "two");
  git("add", ".");
  git("commit", "-m", "fixture two");
  await writeFile(path.join(source, "file.txt"), "dirty");
  await writeFile(path.join(source, "unknown.txt"), "untracked");
  const frozen = path.join(scratch, "frozen"),
    manifest = await snapshotSource(selected, frozen);
  assert.equal(await readFile(path.join(frozen, "file.txt"), "utf8"), "one");
  await assert.rejects(readFile(path.join(frozen, "unknown.txt")));
  assert.notEqual((await resolveSource(source)).commit, manifest.commit);
  assert((await resolveSource(source)).dirty);
  const root = path.join(scratch, "project");
  await createProject(root, { initializeGit: false, host: "fixture" });
  ...
  await releaseConsumer(root, "test", "activation");
  assert.equal((await first.finish()).status, "CLEANED");
  const second = await beginPhase(root, "test", "second");
  await installSoftware(root, frozen, manifest, {
    operationId: "second",
    phase: second,
  });
  let journal = await json(
    path.join(root, "instance/runtime/software-switch.json"),
  );
  await rename(journal.target, journal.candidate);
  await atomic(path.join(root, "instance/runtime/software-switch.json"), {
    ...journal,
    status: "PREPARED",
  });
  await reconcileSoftware(root);
  assert.equal((await verifyInstalledSoftware(root)).commit, manifest.commit);
  await releaseConsumer(root, "test", "activation");
  assert.equal((await second.finish()).status, "CLEANED");
  await writeFile(path.join(root, "review-software/file.txt"), "user edit");
  await assert.rejects(verifyInstalledSoftware(root), /无法确认归属/);
