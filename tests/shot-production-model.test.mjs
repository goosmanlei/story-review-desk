...
test('production default keeps design estimates separate and does not claim locks or actual input versions',()=>{
 ...
 const result=defaultShotProductionPlan(scope,{schemaVersion:'1.0'});assert.deepEqual(result.shots[0].inputs,[]);assert.equal(result.shots[0].keyframeStrategy.mode,'START_ONLY');assert.equal(result.shots[0].videoBranch,'UNKNOWN');assert.equal(result.shots[0].durationSeconds,undefined);assert.equal(result.shots[0].locked,undefined);
});
