...
function fixture(){
 const content={sceneId,identityChangeReason:'初次镜头设计',shots:[1,2].map(i=>({shotId:`${sceneId}-SH0${i}`,sceneId,order:i,title:`镜头${i}`,narrativeBeat:'拿信后产生迟疑',audienceTakeaway:'来信改变决定',materialRequirementRefs:[`MATREQ-${i}`]}))};
 ...
 return {model,scope,content:settings};
}
function readyFixture(){
 ...
 Object.assign(model,compiled);model.shotProductionPlans=[{id:'PRODUCTION-1',sceneId,scopeRole:'CURRENT',sourceRevisionId:'SOURCE-1',content,contentHash:productionHash(content),workItemIds:compiled.workItems.map(w=>w.id)}];
