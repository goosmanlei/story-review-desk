...
function load(name){
 ...
 const profile={instanceId:'fixture',deployment:{runtimeEpoch:'fixture-epoch'}};let cacheModule;
 ...
 const context={module,exports:module.exports,AbortController,DOMException,Response,Error,Event,URL,URLSearchParams,Date,console,structuredClone,confirm:()=>true,fetch(url,options){let resolve,reject;const promise=new Promise((yes,no)=>{resolve=yes;reject=no;});requests.push({url,resolve,reject,signal:options.signal});return promise;},document:{visibilityState:'visible',addEventListener(n,f){listeners.set('document:'+n,f);},removeEventListener(n){listeners.delete('document:'+n);}},window:{location,history,dispatchEvent(event){listeners.get(event.type)?.(event);return !event.defaultPrevented;},confirm:()=>true,addEventListener(n,f){listeners.set(n,f);},removeEventListener(n){listeners.delete(n);},setInterval(f){timers.add(f);return f;},clearInterval(f){timers.delete(f);}},require(id){if(id==='./instance-context')return{useInstanceProfile:()=>profile};if(id==='./workspace-read-cache'){if(!cacheModule){cacheModule={exports:{}};vm.runInNewContext(ts.transpileModule(readFileSync(new URL('workspace-read-cache.ts',sourceRoot),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{...context,module:cacheModule,exports:cacheModule.exports});}return cacheModule.exports;}if(id==='./runtime-path')return{runtimePath:value=>value};if(id==='./creator-production-workflow')return creatorWorkflow;if(id==='react')return react;if(id==='react/jsx-runtime')return{jsx,jsxs:jsx};if(id==='./system-management-client')return{async readManagementResponse(r){const p=await r.json();if(!r.ok)throw Error(p.error);return p;},async managementMutation(...args){writes.push(args);return {};}};if(id==='./runtime-mode')return{useRuntimeMode:()=>({hostedReadOnly:false})};if(id==='./management-draft-guard')return{useManagementDraftGuard(){}};if(id==='./review-semantics')return{visibleText:v=>v};return {};}};
