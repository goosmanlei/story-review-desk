...
async function fixture(page:Page,{readonly=false,conflict=false,query=''}={}){
 if(!bundle){
  const entry=`import React from 'react';import{createRoot}from'react-dom/client';import{ProductionPreparationWorkspace}from'./app/production-preparation-workspace';import{InstanceProfileProvider}from'./app/instance-context';import{useManagementDraftGuard}from'./app/management-draft-guard';function NestedDraft(){const[value,setValue]=React.useState('');useManagementDraftGuard(Boolean(value),'镜头设计');return <input aria-label="镜头设计暂存" value={value} onChange={e=>setValue(e.target.value)}/>;}const workflow=${JSON.stringify(workflow)};function App(){return <InstanceProfileProvider><main className="test-preparation-root"><ProductionPreparationWorkspace workflow={workflow} onStageChange={(phaseId,gateId,creatorStageId,context)=>{const url=new URL(location.href);url.searchParams.set('creatorStage',creatorStageId.toLowerCase().replaceAll('_','-'));url.searchParams.set('productionPhase',phaseId.toLowerCase().replaceAll('_','-'));url.searchParams.set('productionGate',gateId.toLowerCase().replaceAll('_','-'));if(context){url.searchParams.set('preparationEpisode',context.episodeUid);if(context.navigationScopeType==='SCENE'&&context.sceneId)url.searchParams.set('preparationScene',context.sceneId);else{url.searchParams.delete('preparationScene');url.searchParams.delete('scene');}}history.pushState(history.state,'',url);}} renderStage={context=><section aria-label="精确正式工作区接口"><output data-stage-context>{JSON.stringify(context)}</output>{new URLSearchParams(location.search).has('nestedDraft')&&<NestedDraft key={context.sceneId}/>}<p>正式工作区仍须自行核对实际对象；此回归不创建正式对象。</p></section>}/></main></InstanceProfileProvider>}createRoot(document.getElementById('root')).render(<App/>);`;
  const files=buildSync({stdin:{contents:entry,resolveDir:site,sourcefile:'preparation-four-stage-harness.tsx',loader:'tsx'},bundle:true,platform:'browser',format:'iife',jsx:'automatic',write:false,outdir:'virtual-preparation-bundle',logLevel:'silent',define:{'process.env.NODE_ENV':'"production"','process.env.NEXT_PUBLIC_REVIEW_BASE_PATH':'""'}}).outputFiles;
...
test('集场切换和返回保护镜头设计内层草稿，取消时不换场或清空编辑',async({page})=>{
 const f=await fixture(page,{query:'nestedDraft=1&preparationEpisode=episode-first&preparationScene=scene-first'});
 const field=page.getByRole('textbox',{name:'镜头设计暂存'});await field.fill('未保存的运镜说明');
 const original=page.url();let dialog=page.waitForEvent('dialog'),click=episode(page,'episode-second').click();await(await dialog).dismiss();await click;
 await expect(field).toHaveValue('未保存的运镜说明');await expect(page).toHaveURL(original);expect(await context(page)).toMatchObject({sceneId:'scene-first'});
 dialog=page.waitForEvent('dialog');click=episode(page,'episode-second').click();await(await dialog).accept();await click;await expect(field).toHaveValue('');
 await field.fill('第二场待核对');const current=page.url();dialog=page.waitForEvent('dialog');const back=page.goBack();await(await dialog).dismiss();await back;
 await expect(page).toHaveURL(current);await expect(field).toHaveValue('第二场待核对');expect(await context(page)).toMatchObject({sceneId:'scene-second'});clean(f);
});
