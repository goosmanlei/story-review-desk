...
test('hosted component issues no ledger request and shows local-only notice',async({page})=>{
  ...
  const files=buildSync({stdin:{contents:code,resolveDir:path.resolve(import.meta.dirname,'..'),loader:'tsx'},bundle:true,platform:'browser',format:'iife',jsx:'automatic',write:false,define:{'process.env.NODE_ENV':'"production"'}}).outputFiles;
