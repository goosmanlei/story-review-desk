...
import {contextHash,assetContextEventPayload,assetContextCurrentBasis} from '../host/instance-runtime/asset-context-revalidation-model.mjs';
...
async function fixture(t,{sameMillisecond=false}={}){
...
test('actual SQLite -> streamed archive -> fresh import and raw restore preserve exact ledger/source/event and history',async t=>{
 const f=await fixture(t),archive=await f.repo.exportState();validateArchive(archive,f.profile.instanceId);
 ...
 const backup=await f.repo.backupTo(path.join(f.directory,'backup.sqlite')),restored=await restoreInstanceRepository({dbPath:path.join(f.directory,'restored.sqlite'),backupPath:backup.path,expectedSha256:backup.sha256,instanceId:f.profile.instanceId});try{assert.deepEqual((await restored.readView()).eventsByKind['asset-context-revalidation'],[f.event]);}finally{await restored.close();}
});
