...
async function exerciseEpisodeScope(planningVersion = '1.0') {
  ...
  try {
    ...
    const coverageContent={sceneId:scene.id,beats:[{beatId:scene.id+'-B001',order:1,narrativeBeat:'读取来信',audienceTakeaway:'得知消息',visualIntent:'展开信件',actionIntent:'读信',soundIntent:'环境静音',dialogueContext:'无对白',materialRequirementRefs:planningVersion==='2.0'?['requirement:letter']:[]}]};
    ...
    async function planning(kind,content,bindings,id){const v=await repo.readView(),key=kind==='SCENE_COVERAGE'?'sceneCoveragePlanRevisions':'shotPlanSetRevisions',row=v.snapshot.productionModel[key].find(r=>r.scopeId===scene.id);content=api.canonicalSceneScopedContent(v.snapshot,kind,row.planId,content,bindings);if(http)return (await http.post('/api/v8/creative-revisions',{schemaVersion:'2.0',snapshotId:v.snapshot.snapshotId,subjectKind:kind,subjectId:row.planId,content,baseRevisionHash:row.revisionHash,basisBindings:bindings,authorityClass:'A',evidenceRefs:[],note:'隔离测试候选'})).event;const payload={eventId:id,schemaVersion:'2.0',snapshotId:v.snapshot.snapshotId,creativeRevisionId:id,revisionId:id,subjectKind:kind,subjectId:row.planId,scopeType:'SCENE',scopeId:scene.id,content,contentHash:hash(content),baseRevisionHash:row.revisionHash,basisBindings:bindings,basisBindingsHash:hash(bindings),revisionState:'CANDIDATE',adoptionPerformed:false,scopedReviewSpec:api.scopedPlanningReviewSpec(kind,kind==='SHOT_PLAN_SET'?planningVersion:'1.0')};if(kind==='SHOT_PLAN_SET'){if(planningVersion==='2.0')Object.assign(payload,{planningContractVersion:'2.0',materialRequirementSet:api.deriveCurrentMaterialRequirementSet(v.snapshot,scene.id),adoptedMaterialSet:null});else payload.adoptedMaterialSet=api.deriveCurrentAdoptedMaterialSet(v.snapshot,compiler.stateFor(v),scene.id);}payload.contextHash=hash({subjectKind:kind,subjectId:row.planId,baseRevisionHash:row.revisionHash,basisBindingsHash:payload.basisBindingsHash});api.assertCreativeRevisionBasisCurrent(v.snapshot,compiler.stateFor(v),payload);return append('creative-revision',payload);}
    ...
    if(planningVersion==='2.0')assert.throws(()=>api.deriveCurrentAdoptedMaterialSet(view.snapshot,state,scene.id),/素材/);
    const materials=planningVersion==='2.0'?api.deriveCurrentMaterialRequirementSet(view.snapshot,scene.id):api.deriveCurrentAdoptedMaterialSet(view.snapshot,state,scene.id),shotBindings=[{bindingType:planningVersion==='2.0'?'MATERIAL_REQUIREMENT_SET':'ADOPTED_MATERIAL_SET',bindingId:materials.id,bindingHash:materials.contentHash,scopeType:'SCENE',scopeId:scene.id},{bindingType:'SCENE_COVERAGE_REVISION',bindingId:coverage.creativeRevisionId,bindingHash:coverage.contentHash,scopeType:'SCENE',scopeId:scene.id}];
    const shotContent={sceneId:scene.id,identityChangeReason:'首个正式镜头',shots:[{shotId:scene.id+'-SH001',sceneId:scene.id,order:1,title:'读信',coverageBeatRefs:[scene.id+'-B001'],narrativeBeat:'读信',audienceTakeaway:'得知消息',visualIntent:'读信',actionIntent:'读信',soundIntent:'静音',dialogueContext:'无对白',materialRequirementRefs:planningVersion==='2.0'?['requirement:letter']:[],inputBindings:shotBindings}]};
    if(planningVersion==='2.0'){
      const omitted=structuredClone(shotContent);omitted.shots[0].materialRequirementRefs=[];
      assert.throws(()=>api.canonicalSceneScopedContent(view.snapshot,'SHOT_PLAN_SET','SHOT-PLAN-SET:'+scene.id,omitted,shotBindings),/完整覆盖/);
    }
    ...
    const frozen=()=>{const events=Object.values(view.eventsByKind).flat();return {events,snapshot:view.snapshot,binding:{releaseId:view.releaseId,snapshotId:view.snapshot.snapshotId,snapshotSha256:digest(JSON.stringify(view.snapshot)),snapshotCanonicalSha256:digest(view.snapshot),eventManifest:frozenEventManifest(events)}};};
    ...
    if(planningVersion==='2.0'){
      ...
      assert.equal(shot.scopedReviewSpec.schemaVersion,'2.0');
      ...
      produced.productionModel.domainGraph.entities[0].canvasPosition={x:300,y:200};
      assert.deepEqual(api.deriveCurrentMaterialRequirementSet(produced,scene.id),shot.materialRequirementSet);
    }
