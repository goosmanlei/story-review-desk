...
test("object transactions, concurrency, adoption and exact dependency invalidation", async (t) => {
  ...
  await t.test('production manifest and keyframe observation proof survive export and cannot bypass the shared review service',async()=>{
    ...
    const base={subjectType:'WORK_PRODUCT',workItemId:manifest.workItemId,subjectId:manifest.workItemId,versionId:assetId,versionSha256:asset.media.find(m=>m.role==='OUTPUT').sha256,objectRevisionId:asset.revision.id,expectedVersion:asset.version,reviewSpecHash:reviewSpec.hash,criterionFindings:findings,action:'APPROVE_AND_RELEASE',note:'实际输入逐项核对完成'};
