...
test('new AUTHORIZE CLAIM SUBMITTED after exact root re-review creates a usable new descendant without clearing old history',{skip:process.env.REVIEW_TEST_POSTGRES!=='1',timeout:180000},async t=>{
 const f=await apiFixture(t),root=await f.provision(await f.workspace(),'Parent fixture, no provider.'),rootVersion=await f.register(root,await f.beginRun(root),pngs[0]);
 const oldReview=await f.review(root,rootVersion,'APPROVE_AND_RELEASE'),before=await f.repo.readView(),entity=before.snapshot.productionModel.domainGraph.entities.find(r=>r.id==='character:letter-writer');
 await changeDomain(f.repo,'SETTINGS',[{collection:'entities',id:entity.id,beforeHash:domainHash(entity),value:{...entity,description:'Explicit root-only condition B.'}}]);
 const rootReview=await f.review(root,rootVersion,'APPROVE_AND_RELEASE'),changed=await f.repo.readView(),invalidations=structuredClone(changed.snapshot.productionModel.domainInvalidations);
 assert.equal((await f.store.operationalSnapshot()).stateProjection.assetVersionsById[rootVersion.versionId].canFlowDownstream,true);
 const childEntity={id:'style:new-after-review',type:'STYLE',name:'New derived palette',aliases:[],description:'New production only after the root re-review',authority:'A',evidence:[]};
 await changeDomain(f.repo,'SETTINGS',[{collection:'entities',id:childEntity.id,beforeHash:null,value:childEntity}]);
 const requirementId='demand:new-after-review',representation={id:'representation:new-after-review',entityId:childEntity.id,stateId:null,type:'STYLE_ANCHOR',label:'New palette derived from current root',dimensions:{},assetFamilyIds:[],requirementIds:[requirementId],authority:'A',evidence:[]},demand={id:requirementId,title:'New downstream fixture',representationId:representation.id,mediaType:'IMAGE',category:'风格锚点',reuseScope:'PROJECT',scope:[],evidence:[],acceptanceCriteria:['Exact current input and fixed output fixture.']};
 await changeDomain(f.repo,'MATERIAL',[{collection:'representations',id:representation.id,beforeHash:null,value:representation},{collection:'requirements',id:requirementId,beforeHash:null,value:demand}]);
 const child=await provisionRequirement(f,requirementId,[{familyId:root.result.familyId,versionId:rootVersion.versionId,sha256:rootVersion.sha256}]),run=await f.beginRun(child),childVersion=await f.register(child,run,pngs[1]);
 const pending=(await f.store.operationalSnapshot()).stateProjection.assetVersionsById[childVersion.versionId];assert.equal(pending.canFlowDownstream,false);assert.equal(pending.lifecycleState,'REVIEW_PENDING');assert.ok(!pending.flowBlockReasons.includes('DOMAIN_RELATION_INPUT_CHANGED'));
 await approveRequirement(f,child,childVersion,requirementId);
 const final=await f.repo.readView(),state=(await f.store.operationalSnapshot()).stateProjection;
 assert.equal(state.assetVersionsById[rootVersion.versionId].canFlowDownstream,true);assert.equal(state.assetVersionsById[childVersion.versionId].canFlowDownstream,true);
 const requests=final.eventsByKind['execution-request'].filter(e=>e.executionRequestId===run.executionRequestId),runs=final.eventsByKind.run.filter(e=>e.runId===run.runId);
 for(const action of ['AUTHORIZE','CLAIM'])assert.ok(requests.some(e=>e.action===action&&e.eventSequence>rootReview.eventSequence));
 assert.ok(runs.some(e=>e.runState==='SUBMITTED'&&e.eventSequence>rootReview.eventSequence));
 assert.deepEqual(final.snapshot.productionModel.domainInvalidations,invalidations);assert.deepEqual(final.eventsByKind.review.find(e=>e.eventId===oldReview.eventId),oldReview);assert.equal(f.externalCalls(),0);
});
