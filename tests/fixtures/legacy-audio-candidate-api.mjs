...
import {legacyAudioFixture as materialProductionFixture,audioRequirementId as imageRequirementId,params} from '../material-legacy-audio-fixture.mjs';
...
async function apiFixture(t){
 ...
 const beginRun=async p=>{
  ...
  const planned=await post(runs,'v8/runs',{...binding,state:'PLANNED',note:'Synthetic protocol test; no provider invocation.'});assert.equal(planned.status,201,JSON.stringify(planned.body));
  const submitted=await post(runs,'v8/runs',{...binding,runId:planned.body.runId,state:'SUBMITTED',note:'Synthetic protocol test; no provider invocation.'});assert.equal(submitted.status,201,JSON.stringify(submitted.body));
