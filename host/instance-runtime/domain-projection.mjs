...
export function projectDomainGraph(snapshot,graph,reference,{initialization=null,eventVersions=[],preserveReferencePolicies=false,directorySource}={}){
 ...
 const unchangedLegacySemantics=previousGraph&&domainHash(previousGraph)===domainHash(graph)&&Object.entries(previouslyFrozen).every(([id,policy])=>domainHash(policy)===domainHash(policyBindings[id]??null));
 ...
 const context=ids=>{const reps=graph.representations.filter(r=>ids.includes(r.id)),entityIds=new Set(reps.map(r=>r.entityId));const states=graph.states.filter(s=>reps.some(r=>r.stateId===s.id));const entities=graph.entities.filter(e=>entityIds.has(e.id));const relations=graph.relations.filter(e=>ids.includes(e.from.id)||ids.includes(e.to.id)||entityIds.has(e.from.id)||entityIds.has(e.to.id)||states.some(s=>[e.from.id,e.to.id].includes(s.id)));return{hashSchemaVersion:'2.0',representationIds:ids,entityIds:[...entityIds],relationIds:relations.map(r=>r.id),hash:domainHash({representations:reps,states,entities,relations,referencePolicies:relations.filter(r=>r.referencePolicyId).map(r=>({relationId:r.id,policy:policyBindings[r.id]})),representationPolicies:reps.filter(r=>representationPolicies[r.id]).map(r=>({representationId:r.id,policy:representationPolicies[r.id]}))})};};
 for(const r of model.materialRequirements||[])r.domainContext=context(byRequirement.get(r.id)||[]);
 model.domainInvalidations=[...(model.domainInvalidations||[])];for(const f of model.assetFamilies||[]){const old=f.domainContext,next=context(byFamily.get(f.id)||[]);if(old?.hash&&old.hash!==next.hash&&!(old.hashSchemaVersion!=='2.0'&&unchangedLegacySemantics)){const invalidation={familyId:f.id,previousHash:old.hash,currentHash:next.hash,versionIds:[...new Set([...(model.assetVersions||[]),...eventVersions].filter(v=>v.familyId===f.id&&v.sha256).map(v=>v.versionId||v.id))]};if(invalidation.versionIds.length&&!model.domainInvalidations.some(i=>i.familyId===f.id&&i.previousHash===old.hash&&i.currentHash===next.hash))model.domainInvalidations.push(invalidation);}f.domainContext=next;}
