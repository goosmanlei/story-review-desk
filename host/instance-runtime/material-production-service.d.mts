...
export type MaterialProductionPreviewInput = {mode?:'REQUIREMENT_REBASE';requirementId:string;draftRevisionId:string};
export function validateMaterialProductionContent(value:unknown):MaterialProductionContent;
export function getMaterialProductionWorkspace(tx:InstanceReadUnit,input:{requirementId:string;api:MaterialProductionApi;mode?:'REQUIREMENT_REBASE'}):Promise<MaterialProductionWorkspace>;
export function saveMaterialProductionDraft(tx:InstanceUnit,input:{mode?:'REQUIREMENT_REBASE';acknowledgement?:unknown;requirementId:string;expectedReleaseId:string;expectedBasisHash?:unknown;expectedDraftRevisionId:string|null;content:unknown},options:{api:MaterialProductionApi}):Promise<{revisionId:string;formalAdoptionPerformed:false;modelCalls:0}>;
...
export function getMaterialProductionJob(tx:InstanceReadUnit,input:{requirementId:string;jobId:string}):Promise<{schemaVersion:"MATERIAL_PRODUCTION_JOB_STATUS_V1";requirementId:string;jobId:string;mode:string;status:string;error?:string;result?:MaterialProductionPublication}>;
