...
export async function readAssetContextEvidence(tx,model,{view,events}={}){
 ...
 events ||= await tx.listEvents();const metadata=await tx.listPublishedDocumentMetadata(),documents=[],releaseMap=new Map();
...
export async function loadAssetContextRevalidationEvidence(tx,model,options={}){const {rows}=await readAssetContextEvidence(tx,model,options);if(!rows.length&&!Object.hasOwn(model,'assetContextRevalidationEvidence'))return model;const out={...model};delete out.assetContextRevalidationEvidence;if(rows.length)out.assetContextRevalidationEvidence=rows;return out;}
