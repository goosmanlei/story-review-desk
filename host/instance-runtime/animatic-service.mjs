...
export async function assertAnimaticInputs(tx,content,model,{render=false}={}){
 ...
 if(render){const works=currentAnimaticWorks(model,content.sceneId);
  ...
  const dialogue=works.filter(w=>w.deliverableKey==='DIALOGUE_DRY');for(const line of dialogue)if(!content.audio.some(a=>a.role==='DIALOGUE'&&!a.muted&&a.volume>0&&a.lineIds.includes(line.lineId)&&a.media.familyId===line.outputAssetRef))fail('本场适用对白必须完整纳入锁时音轨');
 ...
 const bindings=[];for(const input of animaticMediaBindings(content)){
  ...
  if(!version||version.familyId!==input.familyId||version.sha256!==input.sha256||families.get(input.familyId)?.currentVersionId!==input.versionId||version.scopeRole==='HISTORICAL'||['DO_NOT_USE','RIGHTS_HOLD','REVISION_REQUIRED'].includes(version.lifecycleState)||version.canFlowDownstream!==true)fail('预演输入必须为项目内已放行的精确素材版本');
...
export async function readAnimaticState(tx,{sceneId,...options}={}){
 ...
 const availableMedia=(model.assetVersions||[]).filter(v=>v.canFlowDownstream===true&&v.sha256&&media.some(m=>m.mediaId===v.familyId&&m.versionId===v.id&&m.sha256===v.sha256&&m.availability==='PRESENT'&&m.metadata?.sourceRole!=='ORIGINAL_SOURCE'&&m.metadata?.authorityDomain!=='LOCAL_TRIAL')).map(v=>{const family=(model.assetFamilies||[]).find(f=>f.id===v.familyId);return{familyId:v.familyId,versionId:v.id,sha256:v.sha256,label:family?.label||v.label||v.id,kind:family?.kind||'',path:v.path,mediaUrl:'/api/v8/media/'+token(v.id)};});
 ...
 const jobs=(await tx.listAux(ANIMATIC_NS.jobs)).map(read).filter(j=>j?.sceneId===sceneId).sort((a,b)=>b.createdAt.localeCompare(a.createdAt)).slice(0,30);
 const stale=Boolean(revision&&(!basis||revision.content.shotPlanRevisionId!==basis.shotPlanRevisionId));
 return{sceneId,releaseId:view.releaseId,revisionId:head?.revisionId||null,timelineRevisionId:pointer?.timelineRevisionId||null,content:revision?.content||null,hashes:revision?.hashes||null,impact:revision?.impact||[],stale,blockers,availableMedia,requiredCards,jobs,lock:(model.animaticLocks||[]).find(l=>l.sceneId===sceneId&&l.scopeRole==='CURRENT'&&l.fullSceneCurrent)||null,basis:basis?{sceneId,shotPlanRevisionId:basis.shotPlanRevisionId,shots:basis.shots.map(s=>{const seconds=s.design?.estimatedDurationSeconds??s.durationSeconds;return{shotId:s.shotId||s.id,title:s.title||s.narrativePurpose||'',durationFrames:Number.isFinite(seconds)?Math.max(1,Math.round(seconds*24)):72,durationBasis:Number.isFinite(seconds)?'DESIGN_ESTIMATE':'DRAFT_DEFAULT'};})}:null};
}
