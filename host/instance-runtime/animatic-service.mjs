...
export async function applyAnimaticProjection(tx,model,{view}={}){
 ...
 view ||= await tx.readView();const locks=[];for(const row of await tx.listAux(ANIMATIC_NS.locks)){const lock=read(row),timeline=timelines.find(t=>t.sceneId===lock?.sceneId);if(!lock)continue;const revision=read(await tx.getAux(ANIMATIC_NS.revisions,lock.timelineRevisionId));if(!revision)continue;
