...
export function productionSpaceReasons(model,settings){
 ...
 if(!/^[a-f0-9]{64}$/.test(hash||'')||!source||expected!==hash||![hash,source+'@'+hash].includes(space.freeze))reasons.push('SPACE_FREEZE_NOT_CURRENT');
 ...
 if(zones.length!==1||cameras.length!==1||cameras[0]?.zoneId&&cameras[0].zoneId!==space.zone)reasons.push('SPACE_COORDINATES_NOT_IN_LOCATION');
 ...
 if(states.length!==1||!entity||entity.type!=='LOCATION'||state.authority==='U'||!bound)reasons.push('SPACE_STATE_NOT_BOUND_TO_ADOPTED_INPUT');
 return reasons;
}
