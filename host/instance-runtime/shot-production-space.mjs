...
export function productionSpaceReasons(model,settings,operationalState={}){
 ...
 const leaves=(requirement,input)=>{
  if(!requirement||requirement.requirementClass!=='REQUIRED')return [];
  if(!requirement.composition)return (requirement.assetFamilyRefs||[]).includes(input.familyId)||exactRequirementUsageBinding(operationalState,requirement,input)?[requirement]:[];
  if(!requirementInputFamilyIds(model,operationalState,requirement).includes(input.familyId))return [];
  return requirement.composition.requiredComponents.flatMap(c=>leaves((model.materialRequirements||[]).find(r=>r.id===c.requirementId),input));
 };
 const bound=(settings.inputs||[]).some(input=>leaves((model.materialRequirements||[]).find(r=>r.id===input.requirementId),input).some(requirement=>{
  if(directory?.staleIds?.includes(requirement.id))return false;
