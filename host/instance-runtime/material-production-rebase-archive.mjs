...
export function createMaterialProductionRebaseArchiveValidator(){
 ...
 return {accept(table,row,parsedSnapshot){
  ...
  if(table==='releases'){
   ...
   // Compatibility receipts pin original invalidation indexes. Keep the full
   // ordered sequence in the audit view; filtering would silently renumber it.
   const selected={domainProductionCompatibilities:model.domainProductionCompatibilities||[],domainInvalidations:model.domainInvalidations||[],materialProductionPlans:plans,materialProductionRecipeRevisions:allRows,assetFamilies:(model.assetFamilies||[]).filter(f=>familyIds.has(f.id)),materialWorkItems:(model.materialWorkItems||[]).filter(w=>workIds.has(w.id)),expectedOutputs:(model.expectedOutputs||[]).filter(o=>familyIds.has(o.familyId)),reviewContexts:(model.reviewContexts||[]).filter(c=>requirementIds.has(c.requirementId)),assetVersions:[]};
  ...
  materialProductionRebaseClosures({model:current.model,recipes:current.recipes,documents,initialClosures,validateLegacySegment:materialProductionRevisionClosures,instanceId});
