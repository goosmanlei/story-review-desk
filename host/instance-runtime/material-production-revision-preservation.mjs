...
import {findHistoricalCompatibleRequirementBinding} from './domain-production-compatibility.mjs';
...
export function materialProductionRevisionClosures({model,recipes,documents,initialClosures,instanceId}){
 ...
 const closures=rows.map(row=>{
  ...
  if(row.requirementHash!==plan.requirementHash){
   ...
   // legitimately no longer be current. This history check never grants use.
   ...
   for(const link of declared){
    ...
    const verified=findHistoricalCompatibleRequirementBinding(model,{...link,instanceId});
    if(!verified||verified.eligibility!=='HISTORICAL_ONLY'||(verified.links||[verified]).length!==1||fields.some(k=>(verified.links||[verified])[0][k]!==link[k])||!same(verified.beforeDemand,previousDemand))fail('后继配方的兼容证明与受控历史不同');
    previousHash=link.afterHash;previousDemand=verified.afterDemand;visited.add(previousHash);
   }
   if(previousHash!==row.requirementHash||!same(previousDemand,frozenDemand))fail('后继配方的兼容路径不完整或夹带额外证明');
  }
 ...
 for(const initial of initialClosures){
  ...
  while(true){
   ...
   actualAncestors.set(previousDefinition.id,previousOutput);
   if(c.body.basis.revision.failedAttempt){const p=c.body.basis.revision.failedAttempt,parent=actualAncestors.get(p.parentDefinitionId);if(!parent||parent.id!==p.parentExpectedOutputId||p.parentDefinitionId===previousDefinition.id||c.row.parentVersionId!==c.row.familyId+'@'+parent.plannedVersionLabel)fail('失败重制的实际父版本必须属于已实现的更早祖先');}
   if(c.row.previousExpectedOutputId!==previousOutput.id||c.body.basis.revision.definitionHash!==previousDefinition.definitionHash||c.body.basis.revision.expectedOutputHash!==domainHash(previousOutput)||!c.body.basis.revision.failedAttempt&&c.row.parentVersionId!==c.row.familyId+'@'+previousOutput.plannedVersionLabel||c.output.plannedVersionLabel!=='V'+String(number).padStart(3,'0')||!same(without(c.output,outputMutable),without(previousOutput,outputMutable))||!same(c.body.assetFamily.expectedOutputRefs,[...previousFamily.expectedOutputRefs,c.output.id]))fail('素材后继调用包没有按精确版本和预期产物顺序追加');
