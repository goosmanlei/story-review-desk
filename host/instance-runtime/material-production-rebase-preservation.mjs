 ...
 */
export function materialProductionRebaseClosures({model,recipes,documents,initialClosures,validateLegacySegment,instanceId}){
 ...
 if(!marked)return {revisions:validateLegacySegment({model,recipes,documents,initialClosures,instanceId}),anchors:initialClosures,contextIds:initialClosures.map(c=>c.body.workContext.id)};
 ...
 for(const initial of initialClosures){
  ...
  const flush=()=>{
   ...
   const verified=validateLegacySegment({model:view,recipes,documents,initialClosures:[baseline],instanceId,verifiedAncestors:ancestors.filter(a=>a.definitionId===initial.body.executionDefinition.id||revisions.some(c=>c.definition.id===a.definitionId))});
