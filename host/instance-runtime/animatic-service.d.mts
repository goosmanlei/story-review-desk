...
export type AnimaticState={requiredCards:Array<{requirementId:string;specRevisionId:string;specHash:string;textLines:string[]}>;sceneId:string;runtimeEpoch?:string;releaseId:string;revisionId:string|null;timelineRevisionId:string|null;content:AnimaticTimeline|null;hashes:{timelineHash:string;totalFrames:number}|null;impact:Array<{shotId:string;timingChanged:boolean;visualChanged:boolean;overlayChanged:boolean;boundaryChanged:boolean}>;stale:boolean;blockers:string[];availableMedia:Array<AnimaticMedia&{label:string;kind:string;path:string;mediaUrl:string}>;jobs:Array<{jobId:string;jobRevisionId?:string;status:string;pendingReconciliation?:{reconciliationId:string;status:string}|null;lastReconciliation?:{status:string;outcome?:string;error?:string}|null;reconciliation?:{outcome:string};timelineRevisionId:string;error?:string;result?:{mediaUrl:string;versionId:string;sha256:string}|null}>;lock:{timelineRevisionId:string;reviewEventId:string}|null;basis:{sceneId:string;shotPlanRevisionId:string;shots:Array<{shotId:string;title:string;durationFrames:number}>}|null};
