...
export function workflowOverview(model,preparation,context={}){
 ...
 const productionNodes=CREATOR_PRODUCTION_STAGES.map(stage=>{
  ...
  return {id:stage.id,label:stage.label,group:'全剧制作',order:stage.order,
   ...
   detail:stage.id==='SHOT_BREAKDOWN'?preparationDetail+'；'+stage.purpose:stage.purpose,
 ...
 ...productionNodes];
 const pairs=[['SOURCES','STORY'],['SOURCES','SETTINGS'],['SETTINGS','MATERIALS'],['STORY','SHOT_BREAKDOWN'],['MATERIALS','SHOT_BREAKDOWN'],...productionNodes.slice(0,-1).map((n,i)=>[n.id,productionNodes[i+1].id])];
 ...
 return {schemaVersion:'1.1',nodes,edges,denominatorState:'UNKNOWN',
  ...
  definition:{phases,gates,creatorStages:CREATOR_PRODUCTION_STAGES,creatorProjectionVersion:'1.0'},
