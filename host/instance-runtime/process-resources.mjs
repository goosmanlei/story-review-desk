...
export async function readProcessConfig(root){
 root=await realpath(path.resolve(root));const config=await json(path.join(root,'maintenance/process-cleanup.json'));
...
async function protectedPath(config,absolute){
 ...
 const protectedPaths=[...(config.protectedPaths||[]),config.registry,'maintenance/process-cleanup.json','.git'];
...
export async function beginPhase(root,task,phase,{consumers=[]}={}){
 ...
 for(const relative of config.temporaryRoots||[]){const directory=path.join(config.root,relative);await protectedPath(config,directory);await plain(directory,{create:true});}
...
export async function requiredPhase(selectedPath){
 ...
 if(process.env.REVIEW_SQLITE_OWNER==='CONTAINER')return null;
 let p=path.resolve(selectedPath||process.cwd());
 while(p!==path.dirname(p)){if(await exists(path.join(p,'maintenance/process-cleanup.json')))throw Error('TASK_REQUIRED: use the project task/stage runner before creating process resources');p=path.dirname(p);}
 return null;
}
