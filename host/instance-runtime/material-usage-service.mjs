...
export async function listMaterialUsageSources(tx,{requirementId},{api}){
 ...
 const model=await loadMaterialUsageEvidence(tx,view.snapshot.productionModel,{view}),requirement=materialUsageRequirementBasis(model,requirementId),events=view.eventsByKind||{},state=api.projectOperationalState({...view.snapshot,productionModel:model},events.review||[],events['asset-version']||[],events.run||[],events['source-operation']||[],events['execution-request']||[]),eligibleSources=[];
...
async function context(tx,input,api){
 ...
 const state=api.projectOperationalState(snapshot,events.review||[],events['asset-version']||[],events.run||[],events['source-operation']||[],events['execution-request']||[]);
