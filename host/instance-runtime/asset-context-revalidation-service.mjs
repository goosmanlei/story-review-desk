...
async function context(tx,input,api){
 ...
 const state=api.projectOperationalState(snapshot,e.review||[],e['asset-version']||[],e.run||[],e['source-operation']||[],e['execution-request']||[]),version=model.assetVersions.find(v=>v.id===target.versionId),family=model.assetFamilies.find(f=>f.id===target.familyId);
