...
export function selectShotRecipeInputFamilies(model,work,settings){
 if(work.deliverableKey==='DIALOGUE_TEMP')return{familyIds:[],blockers:work.productionSchemaVersion==='2.0'&&work.stagePolicy==='PREVIS_FIRST_V1'?[]:['SHOT_PRODUCTION_POLICY_REQUIRED']};
...
export function shotRecipeProductionBasis(model,work,plan,inputs){
 ...
 return{...(work.productionSchemaVersion==='2.0'?{productionSchemaVersion:'2.0',stagePolicy:work.stagePolicy,productionPurpose:work.productionPurpose,allowedUse:work.allowedUse}:{}),workItemId:work.id,outputBasisHash:work.outputBasisHash,shotPlanRevisionId:plan.content.shotPlanRevisionId,inputs:inputs.map(input=>Object.fromEntries(Object.entries(input).filter(([key])=>key!=='label'))),...(work.deliverableKey==='SHOT_VIDEO'?{timing,frameSetId:frameSet?.id||null,keyframeMembers:frameSet?.members||[],handles:settings.handles,videoBranch:settings.videoBranch}:['START_FRAME','END_FRAME','INTERMEDIATE_FRAME'].includes(work.deliverableKey)?{timing,keyframeStrategy:settings.keyframeStrategy}:{})};
}
...
export function shotRecipeDefinitionBindingReasons(model,state,definition){
 ...
 for(const familyId of selected.familyIds){const f=state.assetFamiliesById?.[familyId],v=state.assetVersionsById?.[f?.currentVersionId];if(family.kind==='IMAGE'&&f?.kind==='AUDIO')continue;
  const binding={familyId,versionId:v?.id,sha256:v?.sha256};if(productionBindingReasons(model,state,binding,{consumerRole:shotProductionConsumerRole(work,plan)}).length||shotProductionInputConsumptionReasons(model,work,binding).length){reasons.push('SHOT_RECIPE_INPUT_BASIS_CHANGED');continue;}
