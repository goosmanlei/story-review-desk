import {legacyAudioMasterBasis,legacyAudioMasterMetadata,LEGACY_AUDIO_METADATA_SCHEMA} from './material-legacy-audio-metadata.mjs';
...
export async function legacyAudioRevisionContext(tx,{view,model,graph,current,state:operational,requirementId,api}){
 ...
 const basis={requirementId,requirementHash:requirement.requirementHash,requirement,representation,graphRef:{revisionId:current.revisionId,sha256:head.sha256},revision:revisionBasis,voiceMasterMetadata:legacyAudioMasterBasis({graph,representation,origin})};
...
export async function compileLegacyAudioRevision(tx,c,content,draftRevisionId,registeredInput){
 ...
 const executionDefinition={id:definitionId,title:r.family.label,pipelineStageCode:'MATERIAL_PREP',executorKind:'AUDIO_MODEL',definitionStatus:'DEFINED',workItemRef:r.work.id,currentRevisionId:definitionId+':r1',materialRequirementRef:c.requirement.id,materialRequirementHash:c.requirement.requirementHash,legacyMaterialRecipeRevisionId:id,upload:{rawText:'无模型参考；父版本仅作谱系。',items:inputs},model:{branch:content.model,rawRule:content.model,resolution:'JSON'},parameters:p,parametersRaw:canonicalJson(p),prompt:{main:content.prompt,negative:content.negativePrompt,negativeApplication:'APPLY_WITH_MAIN_PROMPT'},output:{path:target,mediaType:'AUDIO',assetFamilyRef:r.family.id,expectedOutputRef:outputId},declaredGate:'READY_TO_START',rawSourceBlock:canonicalJson(authoringContent),authoringContent,parentVersionId:r.parent.versionId,parentVersionSha256:r.parent.sha256,definitionHashSchemaVersion:SHOT_PRODUCTION_DEFINITION_HASH_SCHEMA};
 const master=legacyAudioMasterMetadata({basis:c.basis,sourceBindings:r.origin.sources,previousDefinition:r.definition,definitionId,promptRevisionId:executionDefinition.currentRevisionId,sourcePath,output:expectedOutput,prompt:executionDefinition.prompt});
 seedAsset.metadata=master.metadata;executionDefinition.metadataBinding=master.binding;executionDefinition.seedAssetSha256=domainHash(seedAsset);executionDefinition.definitionHash=executionDefinitionHash(executionDefinition);
