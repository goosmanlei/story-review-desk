...
const selected=events=>events.flatMap(event=>event.eventKind==='creative-revision'&&event.scopedReviewSpec?[{event,role:'CREATION'}]:event.eventKind==='source-operation'&&event.protocol==='SCOPED_SCENE_DATABASE_COMPILER_V1'?[{event,role:'SOURCE_RESULT'}]:[]);
...
export async function captureHistoricalEventContexts(tx,{events,instanceId}){
  ...
  for(const {event,role} of selected(events)){
    ...
    if(role==='CREATION'){
      ...
      release=await tx.readPublishedReleaseAt({recordedBefore:event.recordedAt,snapshotId:event.creationSnapshotId});
...
export function historicalEventContextReader(bundle,{events,expectedHash,instanceId,directory}){
  ...
  for(const {event,role} of required){
    ...
    if(role==='CREATION')requireThat(release.snapshotId===event.creationSnapshotId&&event.snapshotId===event.creationSnapshotId&&Date.parse(release.createdAt)<Date.parse(event.recordedAt),'captured creation publication differs');
