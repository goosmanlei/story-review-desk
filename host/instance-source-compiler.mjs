...
async function compilePinnedInstance({ instanceRoot, documents, activeMedia, retiredMedia, retiredContactMedia, mediaReadScope, events, manifest, profile, baseRelease }, mode) {
  ...
  try {
    ...
    snapshotBytes=Buffer.from(canonicalJson(preserveDomainProjection({snapshot:scoped,baseSnapshot:publishedSnapshot,events})));recipesBytes=Buffer.from(canonicalJson(projected.recipes));
