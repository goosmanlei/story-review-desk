...
export async function* exportRecords(pool) {
  ...
  try {
    ...
    const project = (await tx.query("SELECT instance_id,title FROM project"))
      .rows[0];
    yield JSON.stringify({
      type: "manifest",
      format: "review-project",
      version: 1,
      originInstanceId: project.instance_id,
      title: project.title,
    }) + "\n";
    for (const [table, columns] of Object.entries(TABLES)) {
      const selected = columns
        .map((c) =>
          c === "content_bytes"
            ? "encode(content_bytes,'base64') AS content_bytes"
            : '"' + c + '"',
        )
        .join(",");
      await tx.query(
        `DECLARE transfer_cursor NO SCROLL CURSOR FOR SELECT ${selected} FROM ${table} ORDER BY ${(ORDER_KEYS[table] || columns.slice(0, table === "objects" || table === "revisions" ? 1 : 2)).map((c) => '"' + c + '"').join(",")}`,
      );
      for (;;) {
        const rows = (await tx.query("FETCH FORWARD 100 FROM transfer_cursor"))
          .rows;
        if (!rows.length) break;
        for (const row of rows)
          yield JSON.stringify({ type: "row", table, row }) + "\n";
      }
    ...
    if (!complete) await tx.query("ROLLBACK").catch(() => {});
    tx.release();
  }
}
...
export async function importRecords(
  pool,
  filename,
  { expectedSha256, mediaRoot, operationId } = {},
) {
  check(
    (await fileSha(filename)) === expectedSha256,
    "PACKAGE_HASH",
    "项目数据 SHA 不符",
    409,
  );
  return transaction(
    ...
    async (tx) => {
      ...
      check(
        (await tx.query("SELECT 1 FROM objects LIMIT 1")).rowCount === 0,
        "IMPORT_TARGET_NOT_EMPTY",
        "普通导入只写入空白项目，请在独立新项目中恢复",
        409,
      );
      ...
      try {
        for await (const line of input) {
          check(
            Buffer.byteLength(line) <= 64 * 1024 * 1024,
            "RECORD_TOO_LARGE",
            "单条传输记录超过 64 MiB；请将来源分块",
            413,
          );
          ...
          check(
            entry.type === "row" && Object.hasOwn(TABLES, entry.table),
            "PACKAGE_TABLE",
            "项目包包含未知模块",
          );
          const columns = TABLES[entry.table],
            row = entry.row;
          check(
            row && Object.keys(row).every((c) => columns.includes(c)),
            "PACKAGE_COLUMNS",
            "项目包包含未声明字段",
          );
          if (entry.table === "objects")
            check(
              moduleFor(row.kind).name === row.module,
              "OBJECT_MODULE",
              "对象类型与所属模块不符",
            );
          if (entry.table === "revisions")
            check(
              hash(row.content) === row.sha256,
              "REVISION_HASH",
              "修订正文 SHA 不符",
              409,
              { id: row.id },
          ...
          if (entry.table === "source_documents")
            check(
              ...
              "原始来源字节 SHA 不符",
              409,
              { id: row.original_revision_id },
            );
          if (entry.table === "media" && row.availability === "PRESENT")
            media.push(row);
          const fields = columns.filter((c) => Object.hasOwn(row, c));
          await tx.query(
            `INSERT INTO ${entry.table}(${fields.map((c) => '"' + c + '"').join(",")}) VALUES(${fields.map((c, i) => "$" + (i + 1)).join(",")})`,
            fields.map((c) =>
              c === "content_bytes"
                ? Buffer.from(row[c], "base64")
                : jsonColumns.has(c)
      ...
      for (const row of media) {
        ...
        const filename = path.join(mediaRoot, row.sha256),
          info = await lstat(filename).catch(() => null);
        check(
          info?.isFile() &&
            !info.isSymbolicLink() &&
            info.size === Number(row.byte_size) &&
            (await fileSha(filename)) === row.sha256,
          "MEDIA_INTEGRITY",
          "媒体文件缺失或 SHA 不符",
          409,
          { id: row.id, versionId: row.version_id },
        );
      }
      ...
      const result = {
        ...
        status: "SUCCEEDED",
        formatVersion: 1,
        originInstanceId: manifest.originInstanceId,
        counts,
        verifiedMedia: checked.size,
      };
      if (operationId)
        await tx.query(
