...
export async function mutationGate(tx, runtimeEpoch) {
  ...
  check(
    !runtimeEpoch || project?.runtime_epoch === runtimeEpoch,
    "RUNTIME_CHANGED",
    "实例已切换；请保留草稿并重新打开当前工作区",
    409,
  );
  const marker = (
    await tx.query("SELECT value FROM runtime_status WHERE name='maintenance'")
  ).rows[0];
  check(
    !marker?.value?.enabled,
    "MAINTENANCE",
    "实例正在部署维护，草稿已保留，请稍后保存",
    503,
  );
}
