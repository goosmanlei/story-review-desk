export const commentTargetKey = (target) => `${target.kind}:${target.subjectId}`;
export const commentTargetBinding = (target) => ({ kind: target.kind, subjectId: target.subjectId, revisionId: target.revisionId, planContentHash: target.planContentHash, contentHash: target.contentHash, contextHash: target.contextHash });
...
export function commentItemId(text, index) {
    let hash = BigInt('14695981039346656037');
    for (const char of text)
        hash = BigInt.asUintN(64, (hash ^ BigInt(char.codePointAt(0))) * BigInt('1099511628211'));
    return `${index}-${hash.toString(16)}`;
}
/** The same explicit authored-field catalog drives DOM anchors and server lookup. */
export function storyCommentSources(plan) {
    return plan.content.episodes.flatMap(ep => {
        const d = ep.reviewDossier, blocks = [];
        const add = (id, text, label, groupId) => blocks.push({ id, text, label, groupId });
        ...
        for (const [id, label] of [['episodeTask', '本集任务'], ['characterAction', '人物行动'], ['expressionFocus', '重点表达']])
            add(`purpose.${id}`, d.purpose[id].text, label, 'episode-purpose');
        for (const [id, label] of [['visibleAction', '明线'], ['hiddenTruth', '暗线'], ['audiencePosition', '观众所得']])
            add(`informationLayers.${id}`, d.informationLayers[id].text, label, 'information-causality');
        for (const [id, label] of [['deliveredResult', '本集回报'], ['changedState', '结束状态']])
            add(`payoff.${id}`, d.payoff[id].text, label, 'episode-payoff');
        d.progressionSlices.forEach(s => { for (const id of ['sequenceTitle', 'structuralRole', 'turningPoint', 'audienceGain', 'outputState'])
            add(`slice:${s.sliceId}:${id}`, s[id].text, s.sequenceTitle.text, 'escalation-turn'); });
        ...
        const design = { kind: 'EPISODE_DESIGN', subjectId: ep.episodeUid, episodeUid: ep.episodeUid, label: `${ep.displayId} ${ep.title} · 分集设计`, blocks };
        const scenes = (plan.content.narrativeRevision?.scenes || []).filter(s => ep.sceneIds.includes(s.id)).map(s => ({ kind: 'SCENE_SCRIPT', subjectId: s.id, episodeUid: ep.episodeUid, label: `${ep.displayId} · ${s.displayId} ${s.title}`, blocks: s.scriptBlocks.map(b => ({ id: b.id, text: b.text })) }));
        return [design, ...scenes];
    });
}
export function requirementCommentField(id, sceneId) {
    const fields = { 'episode-task': 'purpose.episodeTask', 'expression-focus': 'purpose.expressionFocus', 'audience-position': 'informationLayers.audiencePosition', 'hidden-truth': 'informationLayers.hiddenTruth', 'episode-payoff': 'payoff.deliveredResult', 'episode-opening': 'openingHook', 'episode-ending': 'endingCliffhanger', 'boundary:opening': 'openingHook', 'boundary:ending': 'endingCliffhanger', 'scene-function': `scene:${sceneId}:function` };
