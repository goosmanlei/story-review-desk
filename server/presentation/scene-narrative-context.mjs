...
export function sceneRequirements(plan, sceneId) {
    const owners = plan.content.episodes.filter(ep => ep.sceneIds.includes(sceneId));
    if (owners.length !== 1)
        throw new Error('此场未唯一绑定到所选分集方案');
    const episode = owners[0], dossier = episode.reviewDossier;
    const requirements = [], missing = [];
    const add = (id, label, claim, scope, sceneIds = [sceneId], blockIds = []) => {
        if (!claim || !claim.text || claim.class === 'U' || claim.text === 'UNKNOWN')
            missing.push(`${label}：${claim?.text || 'UNKNOWN'}`);
    ...
    for (const slice of dossier.progressionSlices.filter(row => row.sceneIds.includes(sceneId))) {
        if (slice.sceneIds.some(id => !episode.sceneIds.includes(id)))
            throw new Error('推进段包含相邻集场次');
        add(`slice:${slice.sliceId}`, `所在推进段：${slice.sequenceTitle.text}`, slice.structuralRole, 'EPISODE', slice.sceneIds);
    }
    for (const beat of dossier.comedyBeats.filter(row => row.sceneIds.includes(sceneId)))
        add(`comedy:${beat.canonicalStoryId}`, '本场笑点与作用', beat.role, 'SCENE', beat.sceneIds);
    if (dossier.schemaVersion === '1.1') {
        for (const [key, label] of [['opening', '本集开场在此场落实'], ['ending', '本集结尾在此场落实']]) {
            const ref = dossier.boundaryEvidence[key];
            if (ref.sceneId === sceneId)
                add(`boundary:${key}`, label, { class: 'A', text: key === 'opening' ? episode.openingHook : episode.endingCliffhanger, evidenceRefs: [] }, 'SCENE', [sceneId], ref.blockIds);
        }
