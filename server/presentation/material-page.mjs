import {soundResourceOwnerSql,soundOwnershipProjection} from '../settings/sound-ownership.mjs';
...
export async function materialPage(unit,params) {
 ...
 const requirementId=params.get('requirementId'),filters=Object.fromEntries(['search','entityId','mediaType','category','ownerId'].map(k=>[k,params.get(k)||null]));
 ...
 const catalog=(await unit.tx.query(`SELECT o.id,o.version,r.id AS revision_id FROM objects o JOIN revisions r ON r.id=COALESCE(o.draft_revision_id,o.adopted_revision_id)
  ...
  AND ($3::text IS NULL OR EXISTS(SELECT 1 FROM memberships m WHERE m.owner_id=o.id AND m.role='ENTITY' AND m.member_id=$3) OR ${soundResourceOwnerSql('o.id','$3')})
  AND ($4::text IS NULL OR COALESCE(r.content->>'mediaType',r.content->>'mediaKind')=$4)
  AND ($5::text IS NULL OR r.content->>'category'=$5)
  AND ($6::text IS NULL OR ${soundResourceOwnerSql('o.id','$6')}) ORDER BY o.id`,[requirementId,filters.search,filters.entityId,filters.mediaType,filters.category,filters.ownerId])).rows;
 const sound=await soundOwnershipProjection(unit,{resourceIds:catalog.map(r=>r.id)});
 const version=hash({catalog,sound:sound.map(b=>[b.id,b.revisionId,b.version])}),filterHash=hash({requirementId,filters});let after='';
