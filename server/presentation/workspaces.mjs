...
export async function operationalProjection(unit,familyId) {
  ...
  const requirementIds=familyId?(await unit.tx.query("SELECT m.owner_id FROM memberships m JOIN objects o ON o.id=m.owner_id WHERE m.role='FAMILY' AND m.member_id=$1 AND o.kind='REQUIREMENT'",[familyId])).rows.map(r=>r.owner_id):undefined;
  const requirements=await materialRows(unit,{ids:requirementIds}),events=await reviewHistory(unit,{id:familyId,limit:familyId?100:5000});
  ...
  const scenes=familyId?[]:await unit.rows(['SCENE'],{content:false}),episodes=familyId?[]:await unit.rows(['EPISODE'],{content:false});
...
export async function workspaceRead(tx, path, params) {
  ...
  else if(name==='views/production-materials'){
    // Filter and paginate the complete lightweight index before reading any selected details.
    const view=await productionPage(unit,new URLSearchParams(),false,{catalog:true}),basis=unit.finish({})._basis;
    const query=parseProductionMaterialQuery(new URL('http://workspace/?'+params),basis);
    result={...queryProductionMaterialPage(view.page,{},query),snapshotId:await unit.namespace(),operationRevision:hash(basis)};
    if(!query.summary){
      const families=result.page.assetFamilies.map(f=>f.id),media=await assets(unit,families);
      result.page.assetFamilies=media.assetFamilies.map(f=>({...f,currentVersionId:f.currentVersionRef,currentExpectedOutputId:f.currentVersionRef?null:f.currentExpectedOutputId}));result.page.assetVersions=media.assetVersions;
      const scenes=await unit.rows(['SCENE'],{ids:result.page.scenes.map(s=>s.id)});
      result.page.scenes=result.page.scenes.map(s=>({...s,...scenes.find(r=>r.id===s.id)?.content}));
      for(const c of result.page.reviewContexts){const scene=scenes.find(s=>s.id===c.scene?.id);if(scene)c.scene.scriptExcerpt=scene.content.text||scene.content.blocks?.map(b=>b.text).join('\n')||'';}
    }
  }
