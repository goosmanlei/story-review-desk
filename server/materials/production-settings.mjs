...
const ref=v=>({familyId:v.familyId,versionId:v.id,revisionId:v.revisionId,sha256:v.sha256});
export async function materialProductionWorkspace(unit,input){
 ...
 const availableInputs=media.assetVersions.filter(v=>v.outputState==='PRESENT'&&v.familyId!==family?.id).map(v=>({...ref(v),label:v.label||media.assetFamilies.find(f=>f.id===v.familyId)?.title||v.id,kind:v.mediaKind,objectVersion:v.objectVersion,eligible:v.canFlowDownstream,blockReasons:v.flowBlockReasons}));
...
function validateContent(content,state,{requireEligible=false}={}){
 ...
 const inputs=content.inputBindings.map(b=>state.availableInputs.find(v=>v.familyId===b.familyId&&v.versionId===b.versionId&&v.sha256===b.sha256&&(!b.revisionId||b.revisionId===v.revisionId)));check(inputs.every(Boolean),'MATERIAL_INPUT_CHANGED','参考素材版本或媒体已改变，请核对精确引用',409);
 if(requireEligible)check(inputs.every(v=>v.eligible),'MATERIAL_INPUT_INELIGIBLE','参考素材尚未采用、已失效或权利不满足；草稿已保留，正式制作前须完成核对',409);
 return inputs;
}
export async function planMaterialProductionChange(tx,input){
 ...
 if(input.action==='save'){
  ...
  const inputs=validateContent(input.content,state);input={...input,content:{...input.content,inputBindings:inputs.map((v,index)=>({familyId:v.familyId,versionId:v.versionId,revisionId:v.revisionId,sha256:v.sha256,order:index+1}))}};assertions.push(...inputs.map(v=>({type:'assert',id:v.versionId,expectedVersion:v.objectVersion})));
 ...
 const inputs=validateContent(value.draft.content,state,{requireEligible:true});assertions.push(...inputs.map(v=>({type:'assert',id:v.versionId,expectedVersion:v.objectVersion})));
