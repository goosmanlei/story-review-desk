...
export async function createStoryProject(project,title){
 ...
 try{
  ...
  stage='SKILLS';await save('CREATING');
  const skillInstall=JSON.parse((await run(process.execPath,[path.join(software,'scripts/instance-skills.mjs'),'install','--project',root,'--software',software],'.project-skills.log')).stdout);
  if(skillInstall.status!=='PROJECT_SKILL_INSTALLED'||skillInstall.coreCommit!==manifest.softwareCommit)throw new Error('Managed project Skill installation proof is missing');
