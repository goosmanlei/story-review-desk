...
export async function runAnimaticWorkerIteration({repository,instanceRoot,workerId,render=renderAnimatic,modelProvider}){
 ...
 let claimed;try{claimed=await repository.writeTransaction(tx=>claimAnimaticRender(tx,jobs[0].jobId,workerId));}catch(e){await repository.writeTransaction(async tx=>{const record=await tx.getAux(ANIMATIC_NS.jobs,jobs[0].jobId),job=read(record);if(job?.status!=='QUEUED')return;await tx.putAux({namespace:ANIMATIC_NS.jobs,key:job.jobId,expectedRevisionId:record.revisionId,bytes:canonicalJson({...job,status:'FAILED',error:String(e.message||e).slice(0,1000),completedAt:new Date().toISOString()})});});return{processed:true,jobId:jobs[0].jobId,status:'FAILED'};}if(!claimed)return{processed:false};let result;
 try{
  await repository.withMediaReadLease(async lease=>{
   await lease.assertHeld();result=await render({repository,instanceRoot,...claimed,assertHeld:()=>lease.assertHeld()});
  ...
  // A materialized file is retained and marked unknown if completion CAS failed.
  const outputPresent=await lstat(path.join(instanceRoot,claimed.job.expectedOutput.targetPath)).then(()=>true,e=>{if(e.code==='ENOENT')return false;throw e;}),status=result||outputPresent?'RESULT_UNKNOWN':'FAILED';
  await repository.writeTransaction(async tx=>{const record=await tx.getAux(ANIMATIC_NS.jobs,claimed.job.jobId),job=read(record);if(job?.workerId!==workerId||job.status!=='RUNNING')return;await tx.putAux({namespace:ANIMATIC_NS.jobs,key:job.jobId,expectedRevisionId:record.revisionId,bytes:canonicalJson({...job,status,error:String(e.message||e).slice(0,1500),result:result||null,completedAt:new Date().toISOString()})});});return{processed:true,jobId:claimed.job.jobId,status};
 }
}
