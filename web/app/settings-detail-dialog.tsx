...
export function SettingsDetailDialog({enabled,children,onRequestClose,label='对象设定详情'}:{enabled:boolean;children:ReactNode;onRequestClose:()=>void;label?:string}){
 ...
 useEffect(()=>{if(!enabled)return;const element=dialog.current;if(!element)return;const origin=document.activeElement instanceof HTMLElement?document.activeElement:null;element.showModal();announceReviewOverlay();return()=>{element.close();announceReviewOverlay();if(origin?.isConnected)origin.focus({preventScroll:true});};},[enabled]);
 if(!enabled)return <>{children}</>;
 if(typeof document==='undefined')return null;
 return createPortal(<dialog ref={dialog} data-review-overlay-host className="settings-detail-dialog" aria-label={label} tabIndex={-1} onCancel={event=>{event.preventDefault();onRequestClose();}} onClick={event=>{if(event.target===event.currentTarget)onRequestClose();}} onKeyDown={event=>{
