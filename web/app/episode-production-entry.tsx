...
import type {ShotDesign} from '../presentation/shot-design-contract.mjs';

type Candidate={expectedVersion?:number;creativeRevisionId:string;contentHash:string;contextHash:string;content:{beats?:Array<Record<string,unknown>>;shots?:Array<Record<string,unknown>>};scopedReviewSpec?:{hash:string;criteria:Array<{id:string;label:string;question:string}>}};
...
export function EpisodeProductionEntry({episodeUid,sceneId}:{episodeUid:string;sceneId:string}) {
  ...
  return <section className="episode-production-entry" aria-label="本集正式镜头设计">
    ...
    {entry&&!entry.release?.canFlowDownstream&&<p>本集尚未正式采用。{entry.release?.reason||'请先在叙事拆解确认本集正文及六项判断。'}</p>}
...
function ScenePlanningReview({plan,sceneId,snapshotId,readOnly,coverage,onSaved}:{plan:Plan;sceneId:string;snapshotId:string;readOnly:boolean;coverage?:{beats?:Array<{beatId:string;narrativeBeat:string;materialRequirementRefs:string[]}>};onSaved:()=>void}) {
  ...
  async function submit() {
    ...
    try {
      const response=await fetch('/api/v1/workspaces/operations/snapshot',{cache:'no-store'}),binding=await response.json() as {snapshotId:string;mutationEtag:string};
      if(!response.ok||binding.snapshotId!==snapshotId||!binding.mutationEtag)throw new Error('运行依据已变化，请重新读取后核对');
      const body={schemaVersion:'2.2',snapshotId,subjectType:'CREATIVE_REVISION',subjectKind:plan.kind,subjectId:plan.template?.planId,objectRevisionId:candidate.creativeRevisionId,expectedVersion:candidate.expectedVersion,subjectRevisionId:candidate.creativeRevisionId,subjectRevisionHash:candidate.contentHash,contextHash:candidate.contextHash,scopeType:'SCENE',scopeId:sceneId,reviewSpecHash:candidate.scopedReviewSpec?.hash,criterionFindings:criteria.map(c=>({criterionId:c.id,verdict:findings[c.id].verdict,note:findings[c.id].note})),action,note};
      const result=await fetch('/api/v1/workspaces/reviews',{method:'POST',headers:{'Content-Type':'application/json','If-Match':binding.mutationEtag,'Idempotency-Key':`scene-review-${crypto.randomUUID()}`},body:JSON.stringify(body)}),payload=await result.json() as {error?:string};
  ...
  return <article className="scoped-planning-card">
    <h4>{title} · {plan.state?.canFlowDownstream?'已采用':plan.review?.action==='APPROVE_AND_RELEASE'?'已采用':candidate?'已登记候选':plan.canAuthor?'可以开始编写':'等待本場上游'}</h4>
    ...
    {candidate&&<><div className="scoped-planning-reading">{(candidate.content.beats||candidate.content.shots||[]).map((row,index)=><section key={index}><h5>{index+1}. {String(row.title||row.narrativeBeat||title)}</h5>{Object.entries(fieldLabels).filter(([field])=>row[field]!==undefined&&field!=='title').map(([field,label])=><p key={field}><strong>{label}：</strong>{Array.isArray(row[field])?(row[field] as string[]).join('、')||'无':String(row[field])}</p>)}{plan.kind==='SHOT_PLAN_SET'&&<ShotDesignSummary design={row.design as ShotDesign|undefined}/>}</section>)}</div>
      {plan.review?<p>{plan.review.action==='APPROVE_AND_RELEASE'?'已登记正式通过。本场采用版本已生效；实际素材输入和生成授权在后续环节核对。':plan.review.action==='REQUEST_REVISION'?'本候选要求修改，请登记新候选。':'本候选禁止使用。'}</p>:criteria.length>0&&<form onSubmit={e=>{e.preventDefault();void submit();}}>
