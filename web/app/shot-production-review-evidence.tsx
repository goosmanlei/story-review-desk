...
import type {ShotProductionEvidence,ShotProductionEvidenceTemplate,ShotProductionFinding} from '../presentation/shot-production-locks.mjs';
...
function Observation({media,kind,checked,disabled,onChange}:{media:ObservationMedia;kind:'IMAGE'|'VIDEO';checked:boolean;disabled:boolean;onChange:(checked:boolean)=>void}){
 ...
 const url=valid?(media.mediaUrl||runtimePath('/api/v1/media/'+media.sha256)):null,error=!valid?'缺少此版本的精确SHA':resolved.key===key?resolved.error:'';
