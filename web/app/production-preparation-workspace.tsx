...
import {usePreparationNavigation,usePreparationReading} from './preparation-navigation';
...
import {SoundOwnershipList,bindingsForObject,type SoundOwnershipBinding} from './sound-ownership';
type Scene={sceneId:string;displayId:string;episodeUid:string;sceneContentHash:string;objectId?:string;revisionId?:string;expectedVersion?:number;sourceSummary?:Record<string,unknown>;preparation:Record<string,unknown>;navigationOnly?:boolean};
type State={releaseId:string;revisionId:string|null;content:{basis:Record<string,unknown>;scenes:Scene[];episodes?:Array<{episodeUid:string;displayId:string;title:string;sceneIds:string[]}>}|null;candidate:{revisionId:string;contentHash:string;episodes:Array<{episodeUid:string;displayId:string;title:string;sceneIds:string[]}>;scenes:Array<{id:string;displayId:string;title:string}>}|null;comments:Array<{sceneId:string;text:string;revisionId:string;preparationRevisionId:string}>;stale:boolean;readOnly?:boolean;soundOwnership?:SoundOwnershipBinding[]};
...
Object.assign(labels,{
 authoringCuts:'作者镜头准备',proposedShotId:'拟定镜头编号',derivedFromAuthoringCutId:'拆分来源镜头',
 actualInputBindings:'实际输入绑定',canGenerate:'是否可生成',dialogueSpeakers:'对白角色',
 materialRequirementRefs:'素材需求引用',requirementDefinitions:'需求定义依据',requirementDefinitionStatus:'需求依据状态',
 requirementHash:'需求哈希',requirementId:'需求身份',pendingDefinitionIds:'待补定义',requirementsNotice:'需求核对说明',scope:'适用范围',
 sourceScriptBlockIds:'正文依据块',sourcePartUse:'正文分段用途',coverageBeatRef:'场级节拍引用',
 cameraIntent:'机位意图',design:'镜头设计',shotSize:'景别',cameraAngle:'机位角度',cameraMovement:'摄影机运动',
 composition:'构图',performance:'表演',lighting:'光线',estimatedDurationSeconds:'预计时长（秒）',subjectIds:'主体身份',
 continuity:'前后连续性',startState:'起始状态',endState:'结束状态',previousShotId:'前一镜头',nextShotId:'后一镜头',
 transitionIn:'入镜衔接',transitionOut:'出镜衔接',keyframeStrategy:'关键帧策略',mode:'方式',
 intermediateFrameCount:'中间关键帧数量',keyframePlan:'关键帧准备',classification:'依据性质',
 start:'开始',end:'结束',frame:'画面',intermediates:'中间关键帧',slot:'帧位置',actualVersionId:'实际素材版本',
 timingStatus:'时长确认状态',localViewAuthorReferences:'局部视图作者依据',
 definitionStatus:'定义状态',proposedRequirementId:'拟定需求身份',definitionResolutionNotice:'定义衔接说明',
 originalUnboundDefinitionEvidence:'原未绑定定义依据',originalUnboundNeed:'原未绑定需求',authoringUses:'作者用途',
 confirmationNeeded:'待确认事项',entityId:'实体身份',proposalKind:'提案类别',resolvedDefinitionBindings:'已定位定义',
});
const preparationValueLabels:Record<string,string>={
 UNKNOWN:'待核',UNBOUND_PROPOSAL:'尚未绑定的准备提案',A:'改编与制作提案',AUTHORING_DRAFT:'准备草稿',
 EXTREME_WIDE:'大远景',WIDE:'远景',FULL:'全景',MEDIUM:'中景',CLOSE_UP:'近景',EXTREME_CLOSE_UP:'特写',
 EYE_LEVEL:'平视',HIGH:'俯拍',LOW:'仰拍',OVER_SHOULDER:'过肩',POV:'主观视角',TOP_DOWN:'垂直俯拍',DUTCH:'倾斜机位',OTHER:'其他',
 STATIC:'固定',PUSH:'推',PULL:'拉',PAN:'水平摇镜',TILT:'垂直摇镜',TRACK:'移',FOLLOW:'跟拍',HANDHELD:'手持',CRANE:'升降',ORBIT:'环绕',
 MULTI_KEYFRAME:'首尾及中间关键帧',START_END:'首尾帧',START_ONLY:'首帧',UNDECIDED:'尚未确定',
 ESTIMATE_ONLY_NOT_ANIMATIC_LOCKED:'仅估时，尚未完成 Animatic 锁时',
 AUTHORING_DESIGN_ONLY_REQUIREMENT_REVALIDATION_PENDING:'作者设计，素材用途待重新核对',
 HISTORICAL_AUTHORING_EVIDENCE_NOT_NEW_INPUT_BINDING:'历史作者依据，尚未建立新输入绑定',
 PUBLISHED_DEFINITION_ONLY_ACTUAL_INPUT_UNBOUND:'已有发布定义，实际输入尚未绑定',
 DEFINITION_OR_CURRENT_INPUT_GAPS_NOT_CLAIM_OF_NO_FILES:'定义或当前输入仍有缺项，实际文件需另行核对',
 AUTHORING_CUT_DEFINITION_USE_ONLY_NOT_FORMAL_SHOT_OR_ACTUAL_MEDIA:'作者准备镜头的定义用途，尚未建立正式镜头及实际素材绑定',
 UNPUBLISHED_REQUIREMENT_DEFINITION:'需求定义尚未发布',
};
function PreparationValue({value}:{value:unknown}){
 if(value==null)return <span>待核</span>;
 if(typeof value==='boolean')return <p>{value?'是':'否'}</p>;
 if(typeof value==='string'||typeof value==='number')return <p>{preparationValueLabels[String(value)]||visibleText(String(value))}</p>;
 if(Array.isArray(value))return <ol>{value.map((v,i)=><li key={i}><PreparationValue value={v}/></li>)}</ol>;
 if(typeof value==='object')return <dl>{Object.entries(value).filter(([k])=>!['meaning','mediaObserved','adoptedAssetBindings'].includes(k)).map(([k,v])=><div key={k}><dt>{labels[k]||k}</dt><dd><PreparationValue value={v}/></dd></div>)}</dl>;
 return null;
}
...
export function ProductionPreparationWorkspace({workflow,initialPhaseId,initialGateId,initialCreatorStageId,renderStage,onStageChange}:Props={}){
 ...
 useEffect(()=>{
  const restore=(event?:PopStateEvent)=>{
   ...
   dirty.current=false;locationSelection.current=next;setSelected(next.sceneId);setSelectedEpisode(next.episodeUid);setStageSelection(next.stageId);setGateSelection(next.gateId);
  };
 ...
 const [pendingDraft,setPendingDraft,draftRetained]=useRetainedDraft<{scene:Scene;comment:string;editing:boolean;draft:Record<string,unknown>}>('preparation:'+(scene?.sceneId||selected||'none'));
 const comment=pendingDraft?.comment||'',editing=pendingDraft?.editing||false,draft=pendingDraft?.draft||{};
 function patchDraft(patch:Partial<NonNullable<typeof pendingDraft>>){if(!scene)return;setPendingDraft(old=>({...old,scene:old?.scene||scene,comment:old?.comment||'',editing:old?.editing||false,draft:old?.draft||{},...patch}));}
 const setComment=(comment:string)=>patchDraft({comment}),setEditing=(editing:boolean)=>patchDraft({editing}),setDraft=(draft:Record<string,unknown>)=>patchDraft({draft});
 useEffect(()=>{dirty.current=Boolean(comment.trim()||editing);},[comment,editing]);
 useManagementDraftGuard(Boolean(comment.trim()||editing)&&!draftRetained,'制作准备稿与意见');
 function leaveDraft(){if((comment.trim()||editing)&&!draftRetained){setError('本机草稿未能保留，请先保存或复制。');return false;}dirty.current=false;return true;}
 ...
 function chooseContext(episodeUid:string,sceneId:string){
  ...
  dirty.current=false;
  if(!episodeMode)navigation.setExpanded(episodeUid,true);
 ...
 function chooseScene(id:string){const next=scenes.find(s=>s.sceneId===id);if(next)chooseContext(next.episodeUid,next.sceneId);}
 function chooseEpisode(uid:string){const nextEpisode=episodes.find(e=>e.episodeUid===uid);if(!nextEpisode)return;const remembered=navigation.lastScene(uid),next=episodeMode?undefined:scenes.find(s=>s.episodeUid===uid&&nextEpisode.sceneIds.includes(s.sceneId)&&s.sceneId===remembered)||scenes.find(s=>s.episodeUid===uid&&nextEpisode.sceneIds.includes(s.sceneId));chooseContext(uid,next?.sceneId||'');}
 function chooseCheck(stageId:CreatorProductionStageId,gateId:string){
  ...
  const nextScene=nextStage.scope==='SCENE'?(scene||episodeScenes.find(s=>s.sceneId===navigation.lastScene(episode?.episodeUid||''))||episodeScenes[0]):undefined;
  ...
  if(onStageChange){
   onStageChange(nextGate.phaseId,nextGate.id,nextStage.id,context);
   // The parent may reject a pending edit or unresolved navigation. Do not
   // switch the visible module until its guarded URL change has succeeded.
   const params=new URLSearchParams(window.location.search);
   if(params.get('creatorStage')!==slug(nextStage.id)||params.get('productionGate')!==slug(nextGate.id))return;
  }else{
   if(!window.dispatchEvent(new Event('review:configuration-before-leave',{cancelable:true})))return;
   const url=preparationSelectionUrl(episode?.episodeUid||'',nextScene?.sceneId||'',nextStage.id,nextGate);if(url.href!==window.location.href)window.history.pushState({...window.history.state},'',url);
  }
 ...
 async function submit(action:'comment'|'save'){if(!state||!scene||!breakdownMode||stageError||state.stale||hostedReadOnly||state.readOnly||selectionError)return;setBusy(true);try{const content=action==='save'?{...state.content,scenes:state.content!.scenes.map(s=>s.sceneId===scene.sceneId?{...s,preparation:draft}:s)}:undefined;const receipt=await managementMutation<{objectId?:string;expectedVersion?:number;revisionId?:string}>('/api/v1/workspaces/production-preparation',{action,objectId:(pendingDraft?.scene||scene).objectId,sceneId:scene.sceneId,expectedVersion:(pendingDraft?.scene||scene).expectedVersion,expectedRevisionId:(pendingDraft?.scene||scene).revisionId,...(action==='comment'?{text:comment}:{preparation:draft})});setPendingDraft(action==='comment'&&editing?{...pendingDraft!,comment:''}:action==='save'&&comment?{...pendingDraft!,scene:{...(pendingDraft?.scene||scene),objectId:receipt.objectId,expectedVersion:receipt.expectedVersion,revisionId:receipt.revisionId},editing:false,draft:{}}:null);setAttempt(v=>v+1);}catch(e){setError(e instanceof Error?e.message:'保存未完成');}finally{setBusy(false);}}
 ...
 return <section className="production-preparation production-flow-workspace creator-production-workspace" aria-label="场景上下文的全剧制作">
  ...
  {!!regularChecks.length&&<nav className={'preparation-step-navigation'+(shotProductionMode?' shot-production-step-navigation':'')} aria-label={stage.label+'子步骤'}>{checkButtons(regularChecks)}</nav>}
  ...
  <div className="preparation-stage-layout">
   <aside className="preparation-scene-directory" ref={directoryReading.pane} tabIndex={0} aria-label={episodeMode?'制作分集目录':'制作集场目录'}>
    ...
    })}{!episodes.length&&<p>尚未建立候选分集。正式制作范围仍待确定。</p>}</nav></div>
   </aside>
   <main className="preparation-stage-workspace" ref={contentReading.pane} tabIndex={0} aria-label="当前制作内容" data-creator-scope={navigationScopeType} data-preparation-current-episode={stageContext?.episodeUid} data-preparation-current-scene={stageContext?.sceneId} data-preparation-current-gate={stageContext?.gateId}>
    <div ref={contentReading.body}><WorkspaceReadBoundary preserveLayout>
	    <header className="preparation-stage-heading"><div><small>{navigationScopeType==='PROJECT'?'全剧导出检查':episodeMode?episode?.displayId:[episode?.displayId,scene?.displayId].filter(Boolean).join(' / ')}</small><h2>{selectionError?'制作上下文需核对':episodeMode?(episode?.title||'选择分集'):source?.title||String(scene?.sourceSummary?.title||'选择本场')}</h2><p>{stage.purpose}</p></div>{scene&&typeof state?.content?.basis.candidateRevisionId==='string'&&<a href={runtimePath('?view=story&storyMode=logic&narrativeLevel=scene&episodePlanRevision='+encodeURIComponent(state.content.basis.candidateRevisionId)+'&episode='+encodeURIComponent(scene.episodeUid)+'&scene='+encodeURIComponent(scene.sceneId))}>阅读对应正文 →</a>}</header>
	    <SoundOwnershipList bindings={bindingsForObject(state.soundOwnership,navigationScopeType==='PROJECT'?instance.episodePlanId:navigationScopeType==='EPISODE'?episode?.episodeUid:scene?.sceneId)} heading={navigationScopeType==='PROJECT'?'本剧制作关联声音':navigationScopeType==='EPISODE'?'本集制作关联声音':'本场制作关联声音'} label="制作范围声音归属"/>
