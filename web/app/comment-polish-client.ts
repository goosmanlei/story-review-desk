'use client';
export type CommentPolishResult={operationId:string;requestId:string;polishedComment:string;model?:string;snapshotId?:string;sceneId?:string;sceneContentHash?:string;businessContextHash?:string;requestMode?:'POLISH_DRAFT'|'SUGGEST_FROM_CONTEXT';sourceContext?:{directBeatCount?:number;relatedBeatCount?:number;missing?:string[]}};
async function read(response:Response){const value=await response.json();if(!response.ok)throw Object.assign(new Error(typeof value.error==='string'?value.error:value.error?.message||`HTTP ${response.status}`),{httpStatus:response.status});return value;}
...
export async function requestCommentPolish(input:unknown,operationId:string,signal?:AbortSignal):Promise<CommentPolishResult>{
  ...
  try{
    ...
    return await pollCommentPolish(operationId,signal);
  }catch(error){if(signal?.aborted)throw error;throw Object.assign(new Error(`${error instanceof Error?error.message:'评论建议未完成'}${submitted?`（操作编号 ${operationId}；未自动重复调用）`:''}`),{httpStatus:(error as {httpStatus?:number}).httpStatus});}
}

export async function pollCommentPolish(operationId:string,signal?:AbortSignal):Promise<CommentPolishResult>{
