...
export function StoryCommentsProvider({plan,episodeUid,sceneId,onReveal,children}: {
  ...
  const {hostedReadOnly:hostReadOnly}=useRuntimeMode(), instance=useInstanceProfile();
  const hostedReadOnly=hostReadOnly||plan.readOnly===true;
  ...
  const url=`/api/v1/workspaces/script-comments?revisionId=${encodeURIComponent(plan.revisionId)}&archive=${plan.archived?'1':'0'}&${sceneId?'sceneId='+encodeURIComponent(sceneId):'episodeUid='+encodeURIComponent(episodeUid||'')}`;
  ...
  async function polish() {
    ...
    try {
      const value=await requestCommentPolish({schemaVersion:'1.2',snapshotId:plan.snapshotId,revisionId:plan.revisionId,target:commentTargetBinding(target),anchor:draft.anchor,commentDraft:draft.text},crypto.randomUUID());
