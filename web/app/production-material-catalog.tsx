...
import {productionMaterialKinds,productionMaterialLocation,type ProductionMaterialFilters,type ProductionMaterialRow} from '../presentation/production-materials.mjs';
import type {ProductionMaterialPage} from '../presentation/production-material-query.mjs';
...
export function ProductionMaterialCatalog({model:baseModel,snapshotId}:{model:ProductionModel;snapshotId?:string}){
  ...
  useEffect(()=>{
    ...
    void fetch('/api/v1/workspaces/views/production-materials?'+query,{cache:'no-store',signal:controller.signal}).then(async response=>{
