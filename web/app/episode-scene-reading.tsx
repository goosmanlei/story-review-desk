...
function SceneReading({ scene, runtime, readOnly }: { scene: ReadingScene; runtime?: Partial<RuntimeEstimate>;readOnly?:boolean }) {
...
function GenericSceneReading({ plan, sceneId, episodeUid }: { plan: EpisodePlanContext; sceneId: string; episodeUid: string }) {
  ...
  return <SceneReading scene={scene} readOnly={plan.readOnly}/>;
}
...
export function EpisodeSceneReading({ plan, sceneId }: { plan: EpisodePlanContext; sceneId: string }) {
  ...
  if (narrative) {
    ...
    return <SceneReading scene={matches[0]} runtime={matches[0].runtime} readOnly={plan.readOnly}/>;
  }
  if(plan.readOnly)return <Unavailable message="这份早期方案未内嵌本场正文；保留原分集设计，不套用当前稿。"/>;
