...
type EditorProps={workItemId:string;definitionRef?:string|null;readOnly?:boolean};
export function ShotProductionRecipeEditor(props:EditorProps){return <RecipeEditor key={props.workItemId} {...props}/>;}
function RecipeEditor({workItemId,definitionRef,readOnly=false}:EditorProps){
 ...
 const draftKey='shot-recipe:'+workItemId,draftRetained=useWorkspaceDraftRetention(draftKey,Boolean(state),dirty,{state,draft,parameters});
 useManagementDraftGuard((dirty&&!draftRetained)||uncertain,'镜头调用包');
 useEffect(()=>{if(!open)return;const controller=new AbortController();fetch('/api/v1/workspaces/shot-production/recipes?workItemId='+encodeURIComponent(workItemId),{cache:'no-store',signal:controller.signal}).then(readManagementResponse<Workspace>).then(result=>{if(controller.signal.aborted)return;const local=readWorkspaceDraft<{state:Workspace;draft:AuthorContent;parameters:string}>(draftKey),content=local?.draft||sourceContent(result);setState(local?.state||result);setDraft(content);setParameters(local?.parameters||JSON.stringify(content.parameters,null,2));setDirty(Boolean(local));setUncertain(false);setPreview(null);setMessage('');}).catch(error=>{if(!controller.signal.aborted)setMessage(error instanceof Error?error.message:'调用包依据不可用');});return()=>controller.abort();},[open,workItemId,refresh]);
 ...
 async function action(action:'save'|'preview'|'publish'){
  ...
  try{
   ...
   const operations=await readManagementResponse<{mutationEtag:string}>(await fetch('/api/v1/workspaces/operations/snapshot?summary=1',{cache:'no-store'}));if(!operations.mutationEtag)throw Error('当前运行快照不可用');
   submitted=true;const response=await fetch('/api/v1/workspaces/shot-production/recipes',{method:'POST',headers:{'Content-Type':'application/json','If-Match':operations.mutationEtag,'Idempotency-Key':requestId},body:JSON.stringify(body)});
   if(response.status>=400&&response.status<500)submitted=false;if(!response.ok)await readManagementResponse(response);
   const result=await response.json() as Preview&{operationId?:string;callId?:string;status?:string;revisionId?:string};
   if(!result||typeof result!=='object'||Array.isArray(result)||(action==='save'?typeof result.revisionId!=='string'||!result.revisionId.trim():action==='preview'?typeof result.previewHash!=='string'||!result.previewHash.trim():typeof result.callId!=='string'||!result.callId.trim()||result.status!=='SUCCEEDED'))throw Error('服务返回的操作回执缺少草稿版本或任务依据');submitted=false;
   ...
   else{setPreview(null);setMessage('调用包新修订已登记：'+result.callId+'。实际输入锁定与生成授权仍分别确认。');window.dispatchEvent(new Event('review:operations-updated'));}
 ...
 return <section className="shot-production-recipe-editor" aria-label="镜头调用包编写">
  {!open?<button type="button" onClick={()=>setOpen(true)}>{definitionRef?'编写调用包新修订':'编写本工作项调用包'}</button>:<>
   ...
   <p>保存作者草稿，核对当前附件与预期输出，确认登记调用定义。模型调用使用独立的生成授权。</p>
   {state?<>
