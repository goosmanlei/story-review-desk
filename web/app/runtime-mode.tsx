...
export function RuntimeModeProvider({ hostedReadOnly, vpsWritable, publicDemo=false, children }: RuntimeMode & { publicDemo?:boolean;children: ReactNode }) {
  return <RuntimeModeContext.Provider value={{ hostedReadOnly, vpsWritable }}>
    ...
    {vpsWritable && <aside className="runtime-mode-banner" role="status"><div><b>{publicDemo?'公开演示站 · 可匿名编辑':'VPS 私有可写审阅台'}</b><span>{publicDemo?'任何访客均可查看和修改，请勿上传隐私或凭据。':''}本次修改只保存在当前远端运行期；下一次发布或回滚会恢复本地生成的干净基线，不回传本地母本。</span></div></aside>}
    {children}
