...
import { CONFIGURATION_SECTIONS, type ConfigurationGroup } from "../presentation/configuration-sections.mjs";
import type {
  Configuration,
} from "../presentation/configuration-model.mjs";
...
export function SystemConfigurationWorkspace() {
  ...
  const retained=useWorkspaceDraftRetention('system-configuration',Boolean(state),dirty,{state,config,draftId,upgradeKeys,savedDraftVisible,group});
  async function load() {
    if (!mayReplace()) return;
    const s = await read<State>(
      await fetch("/api/v1/workspaces/configuration", { cache: "no-store" }),
    );
    setState(s);
  ...
  useEffect(() => {
    let active = true;
    void fetch("/api/v1/workspaces/configuration", { cache: "no-store" })
      .then(read<State>)
      .then((s) => {
        if (active) {
          const draft=readWorkspaceDraft<{state:State;config:Configuration;draftId:string|null;upgradeKeys:string[];savedDraftVisible:boolean;group:Group}>('system-configuration');
          setState(draft?.state||s);setConfig(draft?.config||s.configuration);setDraftId(draft?draft.draftId:s.draft?.revisionId||null);
          if(draft){setUpgradeKeys(draft.upgradeKeys);setSavedDraftVisible(draft.savedDraftVisible);setGroup(draft.group);setDirty(true);}
        }
      })
      .catch((e) => {
  ...
  async function request(path: string, method: string, body: unknown) {
    return Response.json(await managementMutation(path,body,method));
  }
    ...
    <section className="configuration-page" aria-label="系统配置编辑器">
      ...
      <div className="configuration-layout">
        <section className="configuration-editor">
          ...
          {group === "codex" && (
            <>
              <p>项目 Codex 读取当前对象和来源版本；讨论给出建议，执行先展示修改预览，由你确认后保存草稿。助手使用宿主机 Codex 登录态。</p>
              ...
              <fieldset disabled><legend>宿主执行设置（本机维护）</legend><p>后台工作器随审阅台启动，并发为 1。模型、程序位置与凭据引用在本机配置中维护，不随普通项目包导出。</p></fieldset>
              ...
              onClick={() =>
                void action(async () => {
                  const value = await read<{ revisionId: string }>(
                    await request("/api/v1/workspaces/configuration", "PUT", {
              ...
              onClick={() =>
                void action(async () => {
                  const value = await read<Preview>(
                    await request(
                      "/api/v1/workspaces/configuration/preview",
                      "POST",
            ...
            <button
              ...
              onClick={() =>
                void action(async () => {
                  await read(
                    await request(
                      "/api/v1/workspaces/configuration/publish",
                      "POST",
                      {
                        draftRevisionId: draftId,
