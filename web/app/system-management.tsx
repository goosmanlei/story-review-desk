...
function DataAndRuntime({ technicalAppendix }: { technicalAppendix?: ReactNode }) {
  ...
  return <div className="management-stack"><section className="management-card"><header><div><h2>这个故事的数据与运行</h2><p>一部故事使用一个独立实例。备份包括业务版本与受管媒体；恢复到新目录。</p></div></header>
    {hostedReadOnly ? <p>此处是只读镜像。数据维护请回到本地审阅台。</p> : <>
      ...
      <div className="management-actions">{[['verify','核验当前实例'],['backup','创建完整备份'],['export','导出项目数据包']].map(([action,label]) => <button key={action} disabled={busy || !state.value || state.value.capabilities?.[action] === false} onClick={() => void run(action)}>{label}</button>)}<button disabled={state.loading} onClick={() => void state.reload()}>刷新状态</button></div>
