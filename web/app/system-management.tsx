...
const ArchitectureHandbook = dynamic(() => import('./architecture-handbook'), {loading: () => <p role="status">正在读取系统架构手册…</p>});
const tabs = [['start', '使用与初始化'], ['configuration', '系统配置'], ['runtime', '数据与运行'], ['architecture', '系统架构']] as const;
...
export function SourceImport({ onChanged = () => {}, compact=false }: { onChanged?: () => void; compact?:boolean }) {
  ...
  const sources = useManagementLoad<SourceState>('/api/v1/workspaces/sources');
  type ImportDraft={mode:'text'|'file';title:string;role:string;text:string};
  const [draft,setDraft,retained]=useRetainedDraft<ImportDraft>('source-import');
  const current=draft||{mode:'text' as const,title:'',role:'PRIMARY',text:''}, {mode,title,role,text}=current;
  const setMode=(mode:ImportDraft['mode'])=>setDraft({...current,mode}),setTitle=(title:string)=>setDraft({...current,title}),setRole=(role:string)=>setDraft({...current,role}),setText=(text:string)=>setDraft({...current,text});
  const [file, setFile] = useState<File | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  const [message, setMessage] = useState('');
  const readonly = hostedReadOnly || sources.value?.readOnly;
  useManagementDraftGuard(Boolean(file||((title||text)&&!retained)),'待导入资料');
  async function submit() {
    ...
    try {
      if (mode === 'file') {
        ...
        const params=new URLSearchParams({title:title.trim()||file.name,role,originalFilename:file.name,mimeType:file.type||'application/octet-stream'});
        const receipt=await managementMutation<{operationId:string}>('/api/v1/workspaces/sources/file?'+params,file);
        setMessage('来源已排队，正在核验原件；操作编号：'+receipt.operationId);
        await waitForOperation(receipt.operationId);

      } else {
        if (!title.trim() || !text.trim()) throw new Error('填写资料名称和原文后再导入。');
        await managementMutation('/api/v1/workspaces/sources', { title: title.trim(), role, text, expectedReleaseId: sources.value.releaseId });
      }
      setDraft(null); setFile(null); setMessage('资料已登记。原文与后续整理结果分别保留。');
...
function SystemInitialization({openConfiguration}:{openConfiguration:()=>void}) {
  const state=useManagementLoad<{initialized:boolean;readOnly?:boolean}>('/api/v1/workspaces/configuration');
  const {hostedReadOnly}=useRuntimeMode();
  const runtime=useManagementLoad<RuntimeState>('/api/v1/workspaces/maintenance',!hostedReadOnly);
...
function MaintenanceOperation({operation}:{operation:Record<string,unknown>}) {
  ...
  const titles:Record<string,string>={backup:'保存恢复快照',import:'导入备份',restore:'独立恢复',verify:'实例核验',export:'完整项目包导出'};
...
function DataAndRuntime({ technicalAppendix }: { technicalAppendix?: ReactNode }) {
  const { hostedReadOnly } = useRuntimeMode();
  const state = useManagementLoad<RuntimeState>('/api/v1/workspaces/maintenance', !hostedReadOnly);
  const [busy,setBusy] = useState(false);
  const [error,setError] = useState('');
  const [message,setMessage] = useState('');
  ...
  async function run(action: string) {
    setBusy(true); setError(''); setMessage('');
    try { const result = await managementMutation<{ status: string; operationId: string }>('/api/v1/workspaces/maintenance', { action, ...(action === 'restore' ? { backupId: backup, target } : {}) }); setMessage(result.status === 'QUEUED' ? '任务已排队，完成证据会显示在下方。' : '任务状态已更新。'); await state.reload(); }
    catch(e) { setError(e instanceof Error ? e.message : '维护任务未完成'); }
    finally { setBusy(false); }
  }
  return <div className="management-stack"><section className="management-card"><header><div><h2>这个故事的数据与运行</h2><p>一部故事使用一个独立实例。恢复快照保留当前与上一份，可离线恢复；完整导出包保留 24 小时。</p></div></header>
    {hostedReadOnly ? <p>此处是只读镜像。数据维护请回到本地审阅台。</p> : <>
      ...
      <div className="management-actions">{[['verify','核验当前实例'],['backup','保存恢复快照'],['export','导出项目数据包']].map(([action,label]) => <button key={action} disabled={busy || !state.value || state.value.capabilities?.[action] === false} onClick={() => void run(action)}>{label}</button>)}<button disabled={state.loading} onClick={() => void state.reload()}>刷新状态</button></div>
      <MaintenanceTransfer onQueued={()=>void state.reload()}/>
      {!!state.value?.backups?.length && <section className="management-restore"><h3>下载或恢复已核验备份</h3><div className="management-form-grid"><label>已有备份<select value={backup} onChange={e => setBackup(e.target.value)}><option value="">选择一个恢复点</option>{state.value.backups.map((b,index) => <option key={String(b.id || index)} value={String(b.id || b.backupId || '')}>{String(b.title || (b.slot==='current'?'当前快照':b.slot==='previous'?'上一快照':b.createdAt) || b.id || `备份 ${index+1}`)}</option>)}</select></label><label>新实例目录名<input value={target} onChange={e => setTarget(e.target.value)} placeholder="例如：my-story-copy" /></label></div><div className="management-actions"><button disabled={busy || !backup || !target.trim() || state.value.capabilities?.restore === false} onClick={() => void run('restore')}>恢复到新目录</button>{state.value.backups.filter(b=>b.id===backup&&b.downloadUrl).map(b=><a key={String(b.id)} href={runtimePath(String(b.downloadUrl))} download>下载完整备份</a>)}</div><p>恢复不替换当前实例。核验独立副本后，再由本地 Codex 显式切换运行绑定；原实例保留；恢复位置由受管任务回执提供。</p></section>}
...
export function SystemManagement({ technicalAppendix }: { technicalAppendix?: ReactNode }) {
  ...
  return <section className="system-management workspace-view" aria-labelledby="system-management-title">
    ...
    <div id={`management-panel-${tab}`} role="tabpanel" aria-labelledby={`management-tab-${tab}`}>
      ...
      {tabReady && tab==='architecture' && <ArchitectureHandbook />}
    </div>
  </section>;
}
