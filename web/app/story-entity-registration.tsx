...
import type {DomainConfiguration,DomainEntity} from '../presentation/domain-model.mjs';
...
export function StoryEntityRegistration({initial,configuration,busy,onSave,onClose}:{initial:DomainEntity;configuration:DomainConfiguration;busy:boolean;onSave:(entity:DomainEntity)=>Promise<void>;onClose:()=>void}){
  const [draft,setDraft,retained]=useRetainedDraft<{entity:DomainEntity;aliases:string}>('entity-registration:'+initial.type);
  const entity=draft?.entity||initial,aliases=draft?.aliases??initial.aliases.join('，');
  const setEntity=(update:(value:DomainEntity)=>DomainEntity)=>setDraft({entity:update(entity),aliases});
  const setAliases=(aliases:string)=>setDraft({entity,aliases});
  useManagementDraftGuard(Boolean(draft)&&!retained,'主体登记');
  const [error,setError]=useState('');
  const location=initial.type==='LOCATION',title=location?'登记地点':'登记主体';
  async function save(){
    if(busy||!entity.name.trim())return;
    setError('');
    try{await onSave({...entity,name:entity.name.trim(),description:entity.description.trim(),aliases:[...new Set(aliases.split(/[，,；;\n]/).map(value=>value.trim()).filter(Boolean))]});setDraft(null);}
