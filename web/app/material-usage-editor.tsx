...
const endpoint='/api/v1/workspaces/material-usage';
...
function blank(state:MaterialUsageWorkspace):Draft{return {reviewMode:'MATERIAL_OVERALL_V1',purposeNote:'',authorization:{scope:'PROJECT_INTERNAL_ONLY',basis:''},observation:{versionId:state.versionId,sha256:state.sha256,originalViewed:false,note:''},decision:{action:'',reviewSpecHash:state.reviewSpec.hash,criterionFindings:[],note:''}};}
function editable(content:Draft|undefined,state:MaterialUsageWorkspace):Draft{if(!content)return blank(state);if(content.reviewMode==='MATERIAL_OVERALL_V1')return content;return {...blank(state),purposeNote:content.purposeNote,authorization:content.authorization,decision:{...blank(state).decision,note:content.decision.note}};}
function rebaseStaleContent(content:Draft,state:MaterialUsageWorkspace):Draft{const draft=blank(state);return {...draft,purposeNote:content.purposeNote,authorization:content.authorization,decision:{...draft.decision,note:content.decision.note}};}
function complete(draft:Draft,state:MaterialUsageWorkspace){if(!draft.observation.originalViewed||!draft.purposeNote.trim()||!draft.authorization.basis.trim()||!['APPROVE_AND_RELEASE','REQUEST_REVISION'].includes(draft.decision.action)||draft.decision.action==='REQUEST_REVISION'&&!draft.decision.note.trim()||draft.decision.reviewSpecHash!==state.reviewSpec.hash)throw Error('请核对原图和用途授权，选择整体结论；要求修改时须填写整体说明。');return {...draft,reviewMode:'MATERIAL_OVERALL_V1',decision:{...draft.decision,criterionFindings:[]}} as MaterialUsageContent;}
...
function Editor({requirementId}:{requirementId:string}){
 ...
 function loadDraft(next:MaterialUsageWorkspace){setState(next);setDraft(editable(next.draft?.content,next));setDirty(Boolean(next.draft&&next.draft.content.reviewMode!=='MATERIAL_OVERALL_V1'));setPreview(null);}
 useEffect(()=>{if(!open)return;const controller=new AbortController();void (async()=>{
  ...
  else if(local){const next=validUsageWorkspace(await read(await fetch(query(local.target),{cache:'no-store',signal:controller.signal})),local.target);if(controller.signal.aborted)return;loadDraft(next);const current=local.basisHash===next.basisHash&&local.releaseId===next.releaseId,restored=current?editable(local.content,next):rebaseStaleContent(local.content,next);setDraft(restored);setDirty(true);rememberDraft(restored,next);setMessage(current?'已恢复本地未保存草稿。':'已恢复草稿说明；依据已变化，请重新核对整体结论后保存。');}
 ...
 async function inspect(){if(busy)return;setBusy(true);try{
  ...
  if(pending){const result=reconcileUsagePending(pending,next);if(!result.confirmed){setMessage('尚未找到与上次提交完全一致的草稿或任务；保持待核查，未重复提交。');return;}const restored=result.stale?rebaseStaleContent(result.draft!.content,next):result.draft?.content||pending.content;if(result.stale)rememberDraft(restored,next);else instanceSessionStorage.removeItem(draftKey);remember(null);setDirty(!!result.stale);setDraft(restored);setMessage(result.stale?'已核回保存成功；依据已变化，已保留说明，请重新核对整体结论并保存。':result.job?'已核回原请求：'+jobLabels[result.job.status]+'。':'已核回保存的同一份草稿。');}
  else if(dirty&&draft&&state&&(state.basisHash!==next.basisHash||state.releaseId!==next.releaseId)){const restored=rebaseStaleContent(draft,next);rememberDraft(restored,next);setDraft(restored);setMessage('已保留草稿说明；依据已变化，请重新核对整体结论后保存。');}
  else{if(!dirty)setDraft(editable(next.draft?.content,next));setMessage('已重读用途审阅与登记任务。');}
 ...
 async function act(action:'save'|'preview'|'publish'){
  ...
  try{const content=complete(draft,state),target=targetOf(state),requestId='material-usage:'+crypto.randomUUID();
   const operations=await read<{mutationEtag:string}>(await fetch('/api/v1/workspaces/operations/snapshot?summary=1',{cache:'no-store'}));if(typeof operations.mutationEtag!=='string'||!operations.mutationEtag)throw Error('当前素材版本不可用');
   ...
   else{setPreview(null);setState({...state,jobs:[{jobId:String(result.operationId),requestId,status:'SUCCEEDED'},...state.jobs]});setMessage('用途判断已登记并生效，原图采用记录保留。');window.dispatchEvent(new Event('review:operations-updated'));}
  }catch(e){setMessage((e instanceof Error?e.message:'操作失败')+(submitted&&action!=='preview'?'；结果待核查，请重读原请求，勿重复提交。':''));}finally{setBusy(false);}
 }
 const source=selection?.eligibleSources.find(s=>state&&s.familyId===state.familyId&&s.versionId===state.versionId&&s.sha256===state.sha256),imageUrl=source?.mediaToken?runtimePath('/api/v1/media/'+source.mediaToken):source?.mediaUrl;
  ...
  {!open?<button type="button" onClick={()=>{setBusy(true);setOpen(true);}}>复用已通过图片</button>:<>
   ...
   {state&&draft&&<>
    ...
    <fieldset disabled={blocked}>
     ...
     <section aria-label="只读用途审阅标准">{state.reviewSpec.criteria.map(c=><article key={c.id}><b>{c.label}</b><p>{c.question}</p></article>)}</section>
     ...
     <label>用途结论<select value={draft.decision.action} onChange={e=>edit({...draft,decision:{...draft.decision,action:e.target.value as Draft['decision']['action']}})}><option value="">请选择</option>{Object.entries(actionLabels).filter(([action])=>action!=='DO_NOT_USE').map(([action,label])=><option key={action} value={action}>{label}</option>)}</select></label>
     <label>整体审阅说明（要求修改时必填）<textarea aria-label="用途整体审阅说明" value={draft.decision.note} onChange={e=>edit({...draft,decision:{...draft.decision,note:e.target.value}})}/></label>
    </fieldset>
