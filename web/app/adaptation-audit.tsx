...
export type StoryConfirmationTarget = {
  objectRevisionId?:string;expectedVersion?:number;upstreamState?:string;configurationVersion?:number;
...
export function CurrentSceneConfirmationReview({target,snapshotId}:{target:StoryConfirmationTarget;snapshotId:string}) {
  const journeyResult=useSceneJourney(target.objectRevisionId?null:target.sceneId,snapshotId);
...
function SceneConfirmationReview({ target, snapshotId, hardBlockers = [], journeyResult }: { target: StoryConfirmationTarget; snapshotId: string; hardBlockers?: unknown[]; journeyResult: SceneJourneyResult }) {
  ...
  useEffect(() => {
    ...
    const refresh = () => fetch('/api/v1/workspaces/operations/snapshot', { cache: 'no-store', signal: controller.signal })
  ...
  const episodePlanUnavailable = target.objectRevisionId ? target.upstreamState!=='CURRENT' : journeyReasons.some((reason) => [
  ...
  if (!target.objectRevisionId&&journeyResult.loading) submitIssues.push('正在核对本集叙事要求，请稍候。');
  else if (!target.objectRevisionId&&(journeyResult.error || !journeyResult.journey?.storyHandoff)) submitIssues.push('本集叙事依据暂不可用。');
  else if (episodePlanUnavailable) submitIssues.push('请先确认本集叙事要求，再提交本场正文审阅。');
  ...
  async function submit() {
    ...
    try {
      ...
      const request = {
        ...
        objectRevisionId:target.objectRevisionId,expectedVersion:target.expectedVersion,configurationVersion:target.configurationVersion,
...
function AudioVerificationReview({ target, snapshotId }: { target: AudioVerificationTarget; snapshotId: string }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const [etag, key] = await Promise.all([currentMutation(snapshotId), digest(JSON.stringify(request))]);
      const response = await fetch('/api/v1/workspaces/verifications', {
        method: 'POST',
...
function useSceneJourney(sceneId: string | null, snapshotId: string): SceneJourneyResult {
  ...
  useEffect(() => {
    ...
    const readJourney = async (scopeType: 'SCENE' | 'PROJECT', scopeId: string) => {
      const response = await fetch(`/api/v1/workspaces/journeys?scopeType=${scopeType}&scopeId=${encodeURIComponent(scopeId)}`, { cache: 'no-store', signal: controller.signal });
