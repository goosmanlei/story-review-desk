...
const endpoint='/api/v1/workspaces/asset-context-revalidation';
...
function Editor({target,mediaToken}:{target:AssetContextTarget;mediaToken?:string}) {
 ...
 async function act(action:'save'|'preview'|'publish') {if(!state||!draft||blocked)return;setBusy(true);let submitted=false;try{
  ...
  const operations=await read<{mutationEtag:string}>(await fetch('/api/v1/workspaces/operations/snapshot?summary=1',{cache:'no-store'}));if(typeof operations.mutationEtag!=='string'||!operations.mutationEtag)throw Error('当前写入水位不可用');
 ...
 const imageUrl=mediaToken&&/^[A-Za-z0-9_-]+$/.test(mediaToken)?runtimePath('/api/v1/media/'+mediaToken):null;
