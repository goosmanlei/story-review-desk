...
import {readWorkspaceJson,readProductionWorkspace,workspaceCacheScope} from './workspace-read-cache';
...
export default function Home() {
  ...
  const productionRequestsRef = useRef(new Map<string, number>());
  const baseReviewRef=useRef<ReviewSnapshot|null>(null);
  const productionRevisionRef=useRef<string|number|undefined>(undefined);
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/workspaces/views/bootstrap', { signal: controller.signal })
      .then(async (response) => {
  ...
  const loadProduction = useCallback<PagedProductionLoader>(async request => {
    ...
    try {
      ...
      if(data&&payload.snapshotId!==data.snapshotId){
        ...
        data={...fresh.data,productionModel:normalizeProductionModel(fresh.data.productionModel)};
        baseReviewRef.current=data;
        commitPagedProduction({windows:{}});
      }
      if(!data||payload.snapshotId!==data.snapshotId||contexts.some(context=>context.snapshotId&&context.snapshotId!==payload.snapshotId))throw new Error('工作区运行期在读取中变化，请重新完整读取');
      if(request.signal?.aborted)return null;
...
function ReviewApp({ reviewData, pagedProduction, onNeedProduction }: { reviewData: ReviewSnapshot; pagedProduction: PagedProductionDataState; onNeedProduction: PagedProductionLoader }) {
  ...
  const productionModel = useMemo(() => {
    // Paged workspaces already carry the verified operation revision. An older
    // background projection must not overwrite those freshly read row states.
    const projected = activeView==='materials'||activeView==='pipeline' ? reviewData.productionModel : applyOperationalProjection(reviewData.productionModel, operationalProjection);
  ...
  }, [activeView, operationalProjection, reviewData.creativeLineage.spatialEvidence, reviewData.productionModel]);
  ...
  useEffect(() => {
    if(activeView!=='pipeline'&&activeView!=='materials')return;
    const controller=new AbortController();
    void onNeedProduction({resource:activeView==='pipeline'?'production':'materials',filters:activeView==='pipeline'?pipelineFilters:materialFilters,mode:'all',signal:controller.signal});
    return()=>controller.abort();
  },[activeView,onNeedProduction,pipelineFilters,materialFilters]);
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(() => {
      ...
      return fetch('/api/v1/workspaces/views/story-sources', { signal: controller.signal });
    })
      .then(async (response) => {
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    fetch('/api/v1/workspaces/views/adaptation-audit', { signal: controller.signal })
      .then(async (response) => {
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  useEffect(() => {
    if(['materials','pipeline','settings'].includes(activeView))return;
    const controller = new AbortController();
    const refresh = () => {
      fetch('/api/v1/workspaces/operations/snapshot', { signal: controller.signal })
        .then(async (response) => {
          if (!response.ok) throw new Error(`HTTP ${response.status}`);
          return response.json() as Promise<{ stateProjection?: OperationalStateProjection }>;
        })
        .then((payload) => {if(!controller.signal.aborted)setOperationalProjection(payload.stateProjection || null);})
        .catch(() => {
  ...
  }, [activeView,reviewData.snapshotId]);

  const restoreReadRef=useRef<AbortController|null>(null);
  useEffect(()=>()=>restoreReadRef.current?.abort(),[]);
  ...
  locationRestorerRef.current = (historyState = (window.history.state || {}) as ReviewHistoryState, initial = false) => {
    restoreReadRef.current?.abort();
    const restoreController=new AbortController();restoreReadRef.current=restoreController;
      ...
      : ['targets', 'shots'].includes(requestedView || '')
        ...
        : legacyStructureRoute
          ...
          : legacyMapRoute
            ? 'materials'
            : requestedView;
    const nextView = normalizedView && workspaceViews.some((item) => item.id === normalizedView) ? normalizedView as WorkspaceView : (workspaceViews.some(item=>item.id===instance.capabilities.landingView)?instance.capabilities.landingView:'overview') as WorkspaceView;
    ...
    if (needsPagedProduction) {
      ...
      const restorationHref=window.location.href;
      void Promise.all(requests.map((request) => onNeedProduction({...request,signal:restoreController.signal}))).then(async (models) => {
        if(restoreController.signal.aborted||window.location.href!==restorationHref)return;
        const model = models.filter((model):model is ProductionModel=>Boolean(model)).at(-1)||null;
      ...
      ? 'settings' : requested === 'assets' || params.has('asset') ? 'materials'
        : ['targets', 'shots'].includes(requested || '') ? 'pipeline'
          : requested === 'work' ? 'overview' : params.get('materialMode') === 'structure' ? 'story'
            : workspaceViews.some(item => item.id === requested) ? requested : (instance.capabilities.landingView||'overview');
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      fetch(`/api/v1/workspaces/views/search?q=${encodeURIComponent(needle)}&limit=12`, { signal: controller.signal })
        .then(async (response) => {
          if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  function goHome() {
    ...
    saveMaterialBrowseLocation(new URL(window.location.href));
    pendingProductionIntentRef.current=null;
    window.history.pushState({},'', '/');
    setLightbox(null);setSearchResultsOpen(false);setQuery('');setNavigationError('');
    locationRestorerRef.current({},false);
  ...
  function goToView(view: WorkspaceView, pushHistory = true, origin?: NavigationOrigin) {
    ...
    if (view !== activeView && pushHistory) {
      restoreReadRef.current?.abort();pendingProductionIntentRef.current=null;setUrlReady(true);restoringHistoryRef.current=false;
      const url = new URL(window.location.href);
      if(activeView==='materials')saveMaterialBrowseLocation(url);
      cleanDestinationUrl(url);
  ...
  return (
    ...
    <main className="review-shell">
      <aside className="workspace-sidebar" aria-label="审阅台主导航">
        <button className="sidebar-brand" onClick={goHome} aria-label="返回审阅台首页">
      ...
      {activeView === 'story' && <section className="content-section story-section" id="story">
        ...
        {!genericAuthoring && storyView === 'source' && storySourcesReady && <GenericSourceReader onDocumentChange={setGenericSourceFocus} builtin={{documentIds:[storySources.transcript.objectId,storySources.outline.objectId].filter((id):id is string=>Boolean(id)),navigation:(selectBuiltin,builtinActive)=><nav className="source-evidence-tree builtin-source-tree" aria-label="故事来源层级">
              <section className={`source-tree-branch ${builtinActive && storySourceMode === 'transcript' ? 'active' : ''}`}>
                <button aria-pressed={builtinActive && storySourceMode === 'transcript'} onClick={() => { if(!selectBuiltin())return;setStorySourceMode('transcript'); setReaderAnchor(null); }}><span>主工作文本</span><b>完整逐字稿</b><small>{storySources.transcript.segmentCount}段 · 下含{storySources.transcript.sections.length}个时段</small></button>
                <button className="transcript-fold" aria-label={transcriptExpanded?'折叠逐字稿目录':'展开逐字稿目录'} aria-expanded={transcriptExpanded} aria-controls="transcript-sections" onClick={()=>setTranscriptExpanded(value=>!value)}>{transcriptExpanded?'收起各部分 ▴':'展开各部分 ▾'}</button>
                {transcriptExpanded && <div id="transcript-sections" className="transcript-section-list" role="tablist" aria-label="完整逐字稿的各时段">
                  {storySources.transcript.sections.map((item) => <button role="tab" aria-selected={builtinActive && storySourceMode === 'transcript' && transcriptSection.id === item.id} className={builtinActive && storySourceMode === 'transcript' && transcriptSection.id === item.id ? 'active' : ''} key={item.id} onClick={() => { if(!selectBuiltin())return;setStorySourceMode('transcript');setTranscriptSectionId(item.id); setReaderAnchor(null); }}><span>{item.id}</span><b>{item.title.replace(/（.+$/, '')}</b><small>{item.startTimecode}–{item.endTimecode} · {item.segmentCount}段</small></button>)}
                </div>}
              </section>
              <section className={`source-tree-branch auxiliary ${builtinActive && storySourceMode === 'outline' ? 'active' : ''}`}><button aria-pressed={builtinActive && storySourceMode === 'outline'} onClick={() => { if(!selectBuiltin())return;setStorySourceMode('outline'); setReaderAnchor(null); }}><span>辅助对照 · 独立来源</span><b>网友故事梗概</b><small>不是逐字稿父级，不能覆盖一手录音</small></button></section>
            </nav>,player:<section className="source-audio-panel"><div><span>原始录音 · 第一核对源</span><b>{storySources.audio.title}</b><p>{storySources.audio.duration} · {formatBytes(storySources.audio.byteSize)} · 点击任一时间码可从该处回听</p></div><audio ref={storyAudioRef} data-review-audio controls preload="metadata" src={runtimePath(storySources.audio.audioUrl)} onPointerDown={stopSeamAudition} onPlay={(event) => handleReviewAudioPlay(event.currentTarget)}>你的浏览器不支持音频播放。</audio><small>{storySources.audio.reviewUse}</small></section>,content:<section className="text-reader-document">
        ...
        />}
        {!genericAuthoring && storyView === 'story-structure' && !storyOverview && !resolvedEpisodePlan?.content.narrativeRevision && <section className="review-bootstrap-state"><p>当前快照没有可读取的全剧故事结构。</p></section>}
