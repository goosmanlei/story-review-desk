...
export function GenericSourceReader({onDocumentChange,builtin}:{onDocumentChange?:(value:GenericSourceFocus|null)=>void;builtin?:{navigation:(select:()=>boolean,active:boolean)=>ReactNode;content:ReactNode;player?:ReactNode;documentIds:string[]}}){
 ...
 return <div className="text-reader story-source-reader unified-source-workspace">
  <aside className="text-reader-index">
   ...
   {builtin?.navigation(()=>{if(!mayLeaveImport())return false;setSelected(null);setImporting(false);return true;},!selected&&!importing)}
   <nav className="source-evidence-tree instance-source-tree" aria-label="实例来源目录">{sources?.filter(item=>!builtin?.documentIds.includes(item.documentId||'')).map(item=><section key={item.id} className={`source-tree-branch ${selected===item.id&&!importing?'active':''}`}><button aria-pressed={selected===item.id&&!importing} onClick={()=>{if(!mayLeaveImport())return;setSelected(item.id);setImporting(false);}}><span>{({PRIMARY:'原始来源',DERIVED:'整理稿',AUXILIARY:'辅助对照'})[item.role]}</span><b>{item.title}</b><small>{item.format}</small></button></section>)}</nav>
  </aside>
  <div className="source-reader-main">{builtin?.player}{importing&&!readOnly?<article className="text-reader-document source-import-editor"><header className="text-reader-head"><h3>补充来源</h3><button onClick={()=>{if(mayLeaveImport())setImporting(false);}}>返回阅读</button></header><SourceImport compact onChanged={()=>{setImporting(false);setAttempt(v=>v+1);}}/></article>:!selected&&builtin?builtin.content:<article className="text-reader-document"><header className="text-reader-head"><h3>{source?.title||'选择一份来源资料'}</h3></header><div className="text-reader-scroll"><div className="text-reader-body">{!sources&&!error&&<p role="status">正在读取来源目录…</p>}{(error||documentError)&&<><p role="alert">{error||documentError}</p><button onClick={()=>{setError('');setResult(null);setAttempt(value=>value+1);}}>重新读取资料</button></>}{sources&&!sources.length&&<p>尚未导入来源。使用左侧“补充来源”登记最初的故事资料。</p>}{source&&(hostedReadOnly?<p>此镜像展示来源名称与版本信息。原件和逐字内容请回本地核对。</p>:!source.textAvailable?<p>{source.observation==='ORIGINAL_UNOBSERVED'?'原件已保存，尚未观察或转写。':'这份原件尚无可读取文字。'}</p>:document?<><pre className="empty-source-text">{document.text}</pre>{document.truncated&&<p>当前展示前二十万字；完整原文保留在实例中。</p>}</>:!documentError&&<p role="status">正在读取来源正文…</p>)}</div></div></article>}</div>
 </div>;
}
