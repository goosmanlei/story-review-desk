...
export function InlineEvidence({ refs, heading }: EvidenceRequest) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled(requestedRefs.map(async (ref) => {
      const payload=await readWorkspaceJson<{data:EvidenceRecord}>(`/api/v1/workspaces/views/evidence?ref=${encodeURIComponent(ref)}`,workspaceCacheScope(instance),controller.signal);
      return payload.data;
    })).then((results) => {
      if (controller.signal.aborted) return;
...
export function EvidenceReaderProvider({ children, onInternalNavigate }: { children: ReactNode; onInternalNavigate?: (href: string, label: string) => void }) {
  ...
  const [alternatives,setAlternatives]=useState<Record<string,Array<{revisionId:string;sha256:string}>>>({});

  const finishClose = useCallback((restoreFocus = true) => {
    const dialog = dialogRef.current;
    if (dialog?.open) dialog.close();
    versionRequest.current?.abort();setRequest(null);setRecords([]);setAlternatives({});
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    versionRequest.current?.abort();const missingRefs=request.refs;setRecords([]);setErrors({});setAlternatives({});
    setLoading(true);
    Promise.allSettled(missingRefs.map(async (ref) => {
      ...
      if (controller.signal.aborted) return;
      const nextErrors: Record<string, string> = {},choices:Record<string,Array<{revisionId:string;sha256:string}>>={};
      results.forEach((result, index) => {
        if (result.status === 'rejected' && (result.reason as { name?: string }).name !== 'AbortError') {
          if(Array.isArray(result.reason?.details?.versions))choices[missingRefs[index]]=result.reason.details.versions;
          nextErrors[missingRefs[index]] = result.reason instanceof Error ? result.reason.message : '读取失败。';
        }
      });
      setRecords(results.flatMap(r=>r.status==='fulfilled'?[r.value]:[]));setAlternatives(choices);
  ...
  async function chooseVersion(ref:string,revisionId:string){
    versionRequest.current?.abort();const controller=new AbortController();versionRequest.current=controller;
    try{const value=await readWorkspaceJson<{data:EvidenceRecord}>('/api/v1/workspaces/views/evidence?'+new URLSearchParams({ref,sourceRevisionId:revisionId}),workspaceCacheScope(instance),controller.signal);if(controller.signal.aborted)return;setRecords(rows=>[...rows.filter(r=>r.ref!==ref),value.data]);setErrors(old=>{const next={...old};delete next[ref];return next;});}catch(e){if(!controller.signal.aborted)setErrors(old=>({...old,[ref]:e instanceof Error?e.message:'来源读取失败'}));}
  }
      ...
      <div className="evidence-reader-shell">
        ...
        <div className="evidence-reader-body">
          ...
          <section className="evidence-reader-content" aria-live="polite">
            ...
            {selectedRef&&alternatives[selectedRef]?.length>0&&<section><p>此来源保留了多个历史版本，请按原引用选择修订。</p>{alternatives[selectedRef].map(v=><button key={v.revisionId} onClick={()=>void chooseVersion(selectedRef,v.revisionId)}>查看修订 {v.revisionId} · SHA {v.sha256.slice(0,12)}</button>)}</section>}
            {selected && <article data-evidence-ref={selected.ref} data-source-sha256={selected.sourceSha256}>
