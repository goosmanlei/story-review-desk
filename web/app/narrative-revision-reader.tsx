...
export function NarrativeOverview({ plan, tab, onSelectTab }: {plan: EpisodePlanContext; tab: NarrativeOverviewSection; onSelectTab: (tab: NarrativeOverviewSection)=>void}) {
  ...
  return <section className="narrative-overview"><header><h2>故事结构与创作说明</h2><p>{plan.content.episodes.length} 集 · {narrative.scenes.length} 场 · 基准净片长 {runtimeLabel(total.baseSec)}，估算区间 {runtimeLabel(total.compactSec)}—{runtimeLabel(total.spaciousSec)}。{plan.readOnly ? '正在阅读历史方案。' : plan.objectState === 'DRAFT' ? '当前为草稿。' : plan.objectState === 'SUBMITTED' ? '当前为待审稿。' : plan.objectState === 'CHANGES_REQUESTED' ? '当前稿要求修改。' : plan.sourceRole === 'CURRENT' ? '当前为已采用方案。' : '当前为未采用候选。'}</p></header>
