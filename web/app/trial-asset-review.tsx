...
export type TrialAsset = {
  id: string; objectRevisionId?:string;expectedVersion?:number;reviewSpecHash?:string; mediaId: string; versionId: string; sha256: string;
...
export async function loadTrialScopes(signal: AbortSignal): Promise<TrialScopeIndex> {
  const response = await fetch('/api/v1/workspaces/candidates/scopes', { cache: 'no-store', signal });
...
export async function loadTrialWorkspace(signal:AbortSignal,scope:string,prerequisites:string[]) {
  for(let attempt=0;;attempt++) {
    const index=await readWorkspaceJson<TrialScopeIndex>('/api/v1/workspaces/candidates/scopes',scope,signal);
    if(!Array.isArray(index.scopes))throw new Error('试制范围格式不匹配。');
    const urls=index.scopes.map(item=>'/api/v1/workspaces/candidates/snapshot?scopeId='+encodeURIComponent(item.id));
    const [checked,...values]=await readWorkspaceBatch<unknown>(['/api/v1/workspaces/candidates/scopes',...prerequisites,...urls],scope,signal);
...
export async function loadTrialSnapshot(signal: AbortSignal, scopeId?: string): Promise<TrialSnapshot | null> {
  const response = await fetch('/api/v1/workspaces/candidates/snapshot' + (scopeId ? `?scopeId=${encodeURIComponent(scopeId)}` : ''), { cache: 'no-store', signal });
...
export function AssetReview({ asset, etag, refresh, currentVersion, readOnly=false }: { asset: TrialAsset; etag: string; refresh: () => Promise<void>; currentVersion: boolean; readOnly?:boolean }) {
  ...
  async function submit(decision: string) {
    ...
    try {
      const response = await fetch('/api/v1/workspaces/candidates/reviews' + (asset.scopeId ? `?scopeId=${encodeURIComponent(asset.scopeId)}` : ''), {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'If-Match': etag, 'Idempotency-Key': `trial-review-${crypto.randomUUID()}` },
        body: JSON.stringify({
          objectId:asset.id,objectRevisionId:asset.objectRevisionId,expectedVersion:asset.expectedVersion,reviewSpecHash:asset.reviewSpecHash,mediaId: asset.mediaId, versionId: asset.versionId, sha256: asset.sha256, decision, comment,
      ...
      if (!response.ok) {
        ...
        throw new Error(explanations[body.error || ''] || body.message || (typeof body.error==='string'?body.error:'意见暂未保存，请保留内容后重试。'));
      }
