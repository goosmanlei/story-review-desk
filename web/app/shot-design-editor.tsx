...
import {CAMERA_ANGLES,CAMERA_MOVEMENTS,KEYFRAME_STRATEGIES,SHOT_SIZES,canonicalShotDesign,type ShotDesign} from '../presentation/shot-design-contract.mjs';
...
export type ShotDesignPlan={canAuthor:boolean;materialRequirementSetSchemaVersion?:'2.0'|'3.0';basisBindings:Binding[];candidate?:{creativeRevisionId:string;expectedVersion?:number;content:Content};template?:{planId:string;revisionHash:string;content?:Content;retiredShotIds?:string[]}};
...
type EditorProps={plan:ShotDesignPlan;coverage?:Content;sceneId:string;snapshotId:string;readOnly:boolean;onSaved:()=>void};
export function ShotDesignEditor(props:EditorProps){return <ShotDesignEditorInner key={props.sceneId} {...props}/>;}
function ShotDesignEditorInner({plan:livePlan,coverage,sceneId,snapshotId,readOnly,onSaved}:EditorProps) {
 ...
 const [basis,setBasis]=useState(livePlan),[ready,setReady]=useState(false),plan=editing?basis:livePlan,draftKey='shot-design:'+sceneId;
 const retained=useWorkspaceDraftRetention(draftKey,ready,editing,{shots,selected,reason,basis,uncertain});
 useLayoutEffect(()=>{const saved=readWorkspaceDraft<{shots:Shot[];selected:number;reason:string;basis:ShotDesignPlan;uncertain:boolean}>(draftKey);if(saved){setShots(saved.shots);setSelected(saved.selected);setReason(saved.reason);setBasis(saved.basis);setUncertain(saved.uncertain);setEditing(true);}setReady(true);},[draftKey]);
 useManagementDraftGuard(editing&&!retained,'镜头设计');
 const selectedShot=shots[selected],beats=coverage?.beats||[],source=plan.candidate?.content||plan.template?.content;
 function begin(){setBasis(livePlan);setShots(structuredClone(source?.shots||[]).map(shot=>({...shot,design:shot.design||newDesign()})));setReason(source?.identityChangeReason||'NO_IDENTITY_CHANGE');setSelected(0);setEditing(true);setMessage('');setUncertain(false);}
 ...
 async function save(){
  if(!plan.template||!plan.canAuthor||readOnly||busy)return;
  ...
  try {
   ...
   submitted=true;
   const result=await managementMutation<{creativeRevisionId:string}>('/api/v1/workspaces/creative-revisions',{snapshotId,subjectKind:'SHOT_PLAN_SET',subjectId:plan.template.planId,objectRevisionId:plan.candidate?.creativeRevisionId,expectedVersion:plan.candidate?.expectedVersion,baseRevisionHash:plan.template.revisionHash,planningContractVersion:'3.0',...(plan.materialRequirementSetSchemaVersion?{materialRequirementSetSchemaVersion:plan.materialRequirementSetSchemaVersion}:{}),authorityClass:'A',basisBindings:plan.basisBindings,content:{sceneId,identityChangeReason:reason,shots:rows}});
 ...
 return <section className="shot-design-editor" aria-label="结构化镜头设计编辑">
  {!editing?<button type="button" onClick={begin}>{source?.shots?.length?'编辑新镜头设计候选':'编写镜头设计'}</button>:<>
   ...
   <div className="shot-design-actions"><button type="button" disabled={busy||!shots.length} onClick={()=>void save()}>{busy?'正在登记…':uncertain?'查询原操作并完成登记':'登记镜头设计候选'}</button><button type="button" disabled={busy} onClick={()=>{if(confirm('放弃本次未保存镜头设计？'))setEditing(false);}}>取消编辑</button>{uncertain&&<button type="button" onClick={()=>{onSaved();setMessage('已重读当前候选；草稿仍保留，可查询原登记操作。');}}>核查当前候选</button>}</div>
  </>}
