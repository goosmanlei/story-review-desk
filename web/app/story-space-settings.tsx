...
import type {SpatialSettings} from '../presentation/domain-spatial.mjs';
...
export function StorySpaceSettings(){const [state,setState]=useState<SpatialSettings|null>(null),[error,setError]=useState(''),[attempt,setAttempt]=useState(0);useEffect(()=>{const controller=new AbortController();void fetch('/api/v1/workspaces/spatial-settings',{cache:'no-store',signal:controller.signal}).then(readManagementResponse<SpatialSettings>).then(setState).catch(e=>{if(!controller.signal.aborted)setError(e.message);});return()=>controller.abort();},[attempt]);
