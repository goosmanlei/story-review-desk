...
function content(c,t,editable=false){return object(c)&&c.purpose==='LEGACY_ADOPTION_DOMAIN_REVALIDATION'&&(c.reviewMode==='MATERIAL_OVERALL_V1'?['CONFIRM_CURRENT_DOMAIN','REQUEST_CURRENT_DOMAIN_REVISION',...(editable?['']:[])].includes(c.action):c.action==='CONFIRM_CURRENT_DOMAIN')&&c.observedVersionId===t.versionId&&c.observedSha256===t.sha256&&typeof c.note==='string'&&(editable||c.reviewMode==='MATERIAL_OVERALL_V1'&&c.action==='CONFIRM_CURRENT_DOMAIN'||text(c.note))&&Array.isArray(c.criterionFindings)&&new Set(c.criterionFindings.map(f=>f.criterionId)).size===c.criterionFindings.length&&c.criterionFindings.every(f=>text(f.criterionId)&&(editable?['','PASS','FAIL','NA']:['PASS','FAIL','NA']).includes(f.verdict)&&typeof f.note==='string')&&(!editable||typeof c.originalViewed==='boolean');}
...
export function validContextReceipt(v,action,w){
 if(!object(v)||v.modelCalls!==0||v.formalAdoptionPerformed!==false||v.contextRevalidationPerformed!==(action==='publish'))fail();
 if(action==='save'){if(!text(v.revisionId))fail();}
 else if(action==='publish'){if(!text(v.operationId)||v.status!=='SUCCEEDED')fail();}
