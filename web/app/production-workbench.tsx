...
import {RecipeInputLock} from './recipe-input-lock';
...
import type {ShotProductionEvidence} from '../presentation/shot-production-locks.mjs';

import {useInstanceProfile} from './instance-context';
import type { ReviewSpec, ConfigurationBinding, Configuration, ConfigurationRef } from '../presentation/configuration-model.mjs';
import { criteriaForReviewScope } from '../presentation/review-criteria.mjs';
...
export type V7WorkItem = LifecycleProjection & {
  ...
  configurationBinding?: import("../presentation/configuration-model.mjs").ConfigurationBinding;
  id: string;
...
export type MaterialRequirement = {
  ...
  composition?: import('../presentation/material-requirement-composition.mjs').RequirementComposition;
  compositionCoverage?: import('../presentation/material-requirement-composition.mjs').CompositionCoverage;
  materialUsageBindings?: import('../presentation/material-usage-model.mjs').MaterialUsageBinding[];
  sourceKind?: string;
...
export function useExecutionRecipe(recipeId?: string | null) {
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/workspaces/recipes/' + encodeURIComponent(recipeId), { signal: controller.signal })
      .then(async (response) => {
  ...
  useEffect(() => {
    ...
    const eventRequest = versionId
      ? (() => {
        ...
        return fetch(`/api/v1/workspaces/reviews?subjectType=${encodeURIComponent(subjectType)}&subjectId=${encodeURIComponent(subjectId)}&versionId=${encodeURIComponent(versionId)}${contextQuery}&limit=500`, { signal: controller.signal }).then(async (response) => {
...
export function useOriginalMediaUrl(version?: V7AssetVersion | null) {
  ...
  if(!version || version.outputState!=='PRESENT' || !/^[a-f0-9]{64}$/.test(version.sha256||''))return {url:null,error:''};
  const url=version.mediaUrl || runtimePath('/api/v1/media/'+version.sha256);
  return {url:hostedReadOnly&&!url.startsWith('/api/v1/media/')?null:url,error:''};
}
    ...
    try {
      ...
      const response = await fetch('/api/v1/workspaces/reviews', {
        ...
        body: JSON.stringify({
          ...
          subjectType: 'WORK_PRODUCT',
          objectRevisionId:(version as unknown as {revisionId:string}).revisionId,expectedVersion:(version as unknown as {objectVersion:number}).objectVersion,
...
async function currentMutationEtag(snapshotId: string) {
  const response = await fetch('/api/v1/workspaces/operations/snapshot');
...
type RecipeControlsProps={recipe:ExecutionRecipe;context:ExecutionUiContext};
function RecipeExecutionControls(props:RecipeControlsProps){return <RecipeExecutionControlsInner key={props.recipe.id+':'+props.recipe.currentRevisionId} {...props}/>;}
function RecipeExecutionControlsInner({recipe,context}:RecipeControlsProps) {
  const { hostedReadOnly } = useRuntimeMode();
  const configurationProfile=useInstanceProfile();
  const [inputsReady,setInputsReady]=useState(false);
  type Fields={executor:'CODEX'|'USER_EXTERNAL';executionRequestId:string;runEventId:string;resultLabel:string;runId:string;runState:string;providerRunId:string;runNote:string;actualPrompt:string;actualNegativePrompt:string};
  const defaults:Fields={executor:(context.configurationBinding?.productionLane==='CODEX'||(context.configurationBinding?.productionLane!=='USER_EXTERNAL'&&(context.configurationBinding?.defaultExecutor||configurationProfile.capabilities.defaultExecutor)==='CODEX'))?'CODEX':'USER_EXTERNAL',executionRequestId:'',runEventId:'',resultLabel:'',runId:'',runState:'',providerRunId:'',runNote:'',actualPrompt:recipe.prompt.main||'',actualNegativePrompt:recipe.prompt.negative||''};
  const [stored,setStored]=useRetainedDraft<Fields>('recipe-execution:'+recipe.id+':'+recipe.currentRevisionId),fields={...defaults,...stored};
  const {executor,executionRequestId,runEventId,resultLabel,runId,runState,providerRunId,runNote,actualPrompt,actualNegativePrompt}=fields;
  const set=<K extends keyof Fields>(key:K,value:Fields[K])=>setStored(old=>({...defaults,...old,[key]:value}));
  ...
  async function authorize() {
    ...
    try {
      ...
      const body = {
        ...
        action: 'AUTHORIZE',
        workItemId: context.workItemId,executionDefinitionId:recipe.id,callRevisionId:recipe.currentRevisionId,
      ...
      const response = await fetch('/api/v1/workspaces/execution-requests', {
        method: 'POST',
  ...
  async function registerCandidate() {
    ...
    if(!resultFile){setMessage('请选择实际候选文件。');return;}
    if(expectedOutput.familyId!==context.familyId||expectedOutput.id!==recipe.output.expectedOutputRef||expectedOutput.expectationState!=='PLANNED'){setMessage('预期产物与调用包已改变，请重新核对。');return;}
    ...
    try {
      setBusy(true);
      setMessage('正在核对运行并上传已有候选…');
      const current=await fetch('/api/v1/workspaces/execution-requests?executionRequestId='+encodeURIComponent(executionRequestId),{cache:'no-store'}).then(r=>r.json());
      if(current.latestRun?.runId!==runId||current.latestRun?.state!=='SUCCEEDED')throw Error('原运行尚未登记成功');
      const uploadId=crypto.randomUUID(),upload=await fetch('/api/v1/upload?operationId='+uploadId+'&mimeType='+encodeURIComponent(resultFile.type||'application/octet-stream'),{method:'POST',headers:{'Content-Type':'application/octet-stream'},body:resultFile});
      const uploaded=await upload.json();if(!upload.ok)throw Error(uploaded.error||'媒体上传未完成');
      const media=(await waitForOperation(uploaded.operationId)).result;
      if(!media?.mediaId||!media?.sha256)throw Error('上传结果尚未确认，请查询原操作 '+uploaded.operationId);
      const etag = await currentMutationEtag(context.snapshotId);
      const body = {
        snapshotId: context.snapshotId,
        familyId: context.familyId,
        mediaId:media.mediaId,mediaVersionId:media.versionId,sha256:media.sha256,expectedRunEventId:current.latestRun.eventId,
        expectedOutputId: expectedOutput.id,
        label: resultLabel.trim() || undefined,
        note: '由创作者工作台上传并登记已有候选；不代表正式采用。',
        runId: runId.trim(),
      ...
      const semanticKey = await stableDigest(JSON.stringify(body));
      const response = await fetch('/api/v1/workspaces/imports', {
        method: 'POST',
      ...
      if (!response.ok) throw new Error(payload.error || `HTTP ${response.status}`);
      setMessage(`候选已登记为 ${payload.versionId || payload.eventId || '新版本'}；仍为待审候选，未采用或放行。`);
      window.dispatchEvent(new CustomEvent('review:operations-updated'));
    } catch (reason) {
  ...
  async function reportExternalRun(state: 'SUBMITTED' | 'SUCCEEDED' | 'FAILED' | 'RESULT_UNKNOWN') {
    ...
    try {
      ...
      const body = {
        ...
        state,expectedRunEventId:runEventId||undefined,
        providerRunId: providerRunId.trim() || undefined,
        note: runNote.trim() || undefined,
      };
      const semanticKey = await stableDigest(JSON.stringify(body));
      const response = await fetch('/api/v1/workspaces/runs', {
        method: 'POST',
      ...
      setRunState(payload.state || state);setRunEventId(payload.eventId||'');
  ...
  return <section className="creator-execution-controls">
    ...
    <RecipeInputLock callId={recipe.id} onReady={setInputsReady} disabled={hostedReadOnly||busy}/>
    <button type="button" className="creator-authorize-button" disabled={hostedReadOnly || busy || !context.familyId || !inputsReady} onClick={() => void authorize()}>{busy ? '正在处理…' : hostedReadOnly ? '远端镜像不可授权生成' : `标记可生成 · ${executor === 'CODEX' ? 'Codex执行' : '我在外部执行'}`}</button>
    {!inputsReady && <p className="creator-execution-gate">核对并锁定当前实际输入后可授权。</p>}
    {executor === 'USER_EXTERNAL' && executionRequestId.startsWith('xreq_') && <section className="creator-external-run"><header><div><b>记录外部平台运行</b><span>{runState ? `当前：${visibleText(runState)}` : '尚未登记提交'}</span></div><code>{executionRequestId}</code></header><label><span>平台任务ID（必填）</span><input value={providerRunId} onChange={(event) => setProviderRunId(event.target.value)} placeholder="historyId / requestId / taskId" /></label><label><span>情况说明（失败／结果不明必填）</span><textarea value={runNote} onChange={(event) => setRunNote(event.target.value)} placeholder="记录平台提示、核查入口或失败原因；不要粘贴密钥" /></label><div>{!runId && <button type="button" disabled={hostedReadOnly || busy} onClick={() => void reportExternalRun('SUBMITTED')}>已提交到外部平台</button>}{runId && ['SUBMITTED', 'RESULT_UNKNOWN', ''].includes(runState) && <><button type="button" disabled={hostedReadOnly || busy} onClick={() => void reportExternalRun('SUCCEEDED')}>已有明确结果</button><button type="button" disabled={hostedReadOnly || busy} onClick={() => void reportExternalRun('FAILED')}>执行失败</button><button type="button" disabled={hostedReadOnly || busy} onClick={() => void reportExternalRun('RESULT_UNKNOWN')}>结果不明</button></>}</div>{runId && <p>本次Run：<code>{runId}</code>{runState === 'SUCCEEDED' ? ' · 可继续登记候选' : ''}</p>}</section>}
    <details className="creator-candidate-import"><summary>登记已有候选结果</summary><p>上传实际候选文件，经后台核验 SHA 后登记为待审版本。必须绑定已授权请求和已成功Run；附件、模型和参数若偏离已授权调用包，应先更新配方并重新授权。</p><label><span>实际候选文件</span><input type="file" disabled={busy} onChange={event=>setResultFile(event.target.files?.[0]||null)} /></label><div><label><span>候选标签（可选）</span><input value={resultLabel} onChange={(event) => setResultLabel(event.target.value)} placeholder="例如：返修V002" /></label><label><span>Run ID（必填）</span><input value={runId} onChange={(event) => setRunId(event.target.value)} placeholder="run_…" /></label></div><label><span>Execution Request ID（必填）</span><input value={executionRequestId} onChange={(event) => setExecutionRequestId(event.target.value)} placeholder="xreq_…" /></label><button type="button" disabled={busy||!executionRequestId} onClick={()=>void queryExecution()}>查询原请求与运行状态</button><label><span>实际使用的完整主Prompt</span><textarea value={actualPrompt} onChange={(event) => setActualPrompt(event.target.value)} /><small>若在外部平台改过Prompt，必须用最终实际文本替换这里的配方原文。</small></label><label><span>实际使用的完整负面Prompt</span><textarea value={actualNegativePrompt} onChange={(event) => setActualNegativePrompt(event.target.value)} /><small>没有负面Prompt时保持为空；不得把改动只留在平台历史中。</small></label><button type="button" disabled={hostedReadOnly || busy || !context.familyId} onClick={() => void registerCandidate()}>{busy ? '正在处理…' : hostedReadOnly ? '远端镜像不可登记候选' : '预检并登记已有结果'}</button></details>
