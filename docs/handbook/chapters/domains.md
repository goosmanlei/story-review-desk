...
具体人物使用独立永久 ENTITY，类型为 CHARACTER。`isExtra` 是 `true`、`false`、`null` 三态属性，省略投影为 UNKNOWN；名称或素材分类不决定是否群演，非人物不能写入该字段。历史 GROUP 身份继续保留，实例纠正须另行迁移。REQUIREMENT 与 MATERIAL 可无单主体登记，`representationId` 可空，组合素材不必创建替代人物或群体。
