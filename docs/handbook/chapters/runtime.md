...
macOS 由 LaunchAgent 托管，Linux 由 systemd 托管。服务监督器启动 Web 和工作器，并管理有限日志与中断后的任务清理。每个数据库池最多 8 个连接；业务工作器执行并发为 1，排队上限为 100。

正式任务由项目 Codex 主会话协调，使用项目专属 App Server 执行软件派工。服务缺失时受管启动、健康时复用，执行身份及历史保存在本机 runtime；最多占用 3 个不同正式任务，并按实际 Agent 容量和资源互斥调度。它独立于业务工作器和公共 Codex daemon；模型执行不会由网页阅读或项目复制自动启动，具体生命周期见正式任务管理。
...
实例首次加载、暂时读取失败以及部署或运行期变化，使用统一的居中状态卡片。加载中自动等待连接；失败可查看原因并重新加载；旧页面发现更新后暂停工作区交互，提示刷新。按钮支持键盘焦点与激活、悬停和点击反馈，窄屏正常排布，减少动态效果偏好下停止旋转动画。

刷新保留当前地址，重新读取实例信息后才进入工作区。状态页只呈现原有实例检测结果，不写入故事对象，也不改变请求携带的运行期和服务端版本校验。草稿存储继续按实例和运行期隔离：同一工作区正常刷新仍可保留草稿，工作区重新载入后不自动带入旧运行期草稿。
