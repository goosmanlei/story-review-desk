...
`SPATIAL_PLACEMENT_DRAFT` NOTE 只保存待确认方案，位置槽位仅是画布显示记录。保存核对 NOTE 版本与草稿头，并绑定地点、当前空间来源及实际场依据的精确修订。再次编辑可追加说明或显示方案，不能换绑这些依据。

依据变化时保留陈旧原稿供核对；重复当前提案和已有空间锚点冲突不自动择一。此角色不开放通用保存、提交、采用、禁用、归档或自动替代路径，因此不能靠去掉角色字段使待确认位置成为正式设定。

## 松散用途的独立版本

素材松散用途 NOTE 与素材版本用途审阅各自保存。编辑先保存独立编辑草稿，经预览后应用为业务 DRAFT；不提交或采用素材，不改变权利或冻结输入。每条用途使用自身版本和草稿头，不能换绑素材族；移除归档原 NOTE，旧身份与范围修订仍可读取，已归档身份不可复用。

用途不开放普通 submit、review 或 disable，不能被其他对象引用为正式输入依赖。依据漂移保留原记录并显示 STALE；用途变更不会向实际制作依赖传播失效。具体 CAS、预览与移除契约见[接口契约](interfaces.md#素材松散用途接口)。
