...
制作过程素材的制作步骤、素材种类、媒介、状态、分集和场景筛选按组完整平铺为按钮，包含各组“全部”，单组选一项、跨组组合，选项多时自动换行。点选沿用搜索与精确身份筛选；更换分集清除场景及镜头条件，更换场景清除镜头条件。同值点击不改变条件。当前地址中的条件未出现在可选目录时，保留明确的当前筛选按钮；地址、刷新及前进后退继续恢复条件。
