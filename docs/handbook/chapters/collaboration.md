...
实体详情内的局部图整合相邻实体、素材表现与关联需求。素材节点使用实际版本缩略图；音频仅标明类型，媒体缺失或预览读取失败明确显示。点击素材只展开卡内要求、状态和采用版本，不跳转页面；采用版本与待审版本不能混称。来源、永久身份、当前及采用修订、适用范围在折叠追溯区回读。实体评论继续绑定原修订，页面布局与筛选不改写业务数据。
