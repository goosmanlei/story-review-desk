...
`--check` 只读检查计划，并显示当前发布的 Bridge 启动计划。实际启动前核对端口归属；隔离验收可选另一端口，不替换不属于本实例的容器。`--offline` 停用本实例的可选 AI 审阅工作器，不要求模型密钥，也不改变 Codex Bridge 的宿主 CLI 认证。运行器会启动宿主维护工作器；系统配置启用 Bridge 自动启动时，还会启动或核对本实例唯一的宿主托管器。托管器通过实例锁、进程身份、release、runtime epoch 与配置哈希防止误复用，异常时有限退避并只让页面助手降级。
...
系统管理包含使用与初始化、系统配置、数据与运行。系统配置按项目设定、资料与设定、素材与制作、审阅标准、AI 配置组织；项目设定合并项目画面与界面，AI 配置再分 AI 助手、项目 Codex 和暂时独立的试制入口。默认值经草稿、预览与发布生效；对象冻结既有规则，显式升级受依赖检查保护。关系默认规则变化作用于后续确认，既有确认关系要重新预览并发布才能采用新规则。详见 [系统配置](system-configuration.md)。

实体图谱说明故事身份、状态和预期参考；实际生产输入另以资产族、版本、SHA 和冻结记录证明。亲缘关系不自动授权身份参考，参考声明不证明历史调用已使用附件。界面、助手和制作门禁都保留这些区别。

本机 Codex Bridge 可以检查连接和读取冻结上下文。其自动启动、模型、并发和空闲回收时间在“系统配置 → AI 配置 → 项目 Codex”按实例发布；配置生效以随后一次本地实例启动及 Bridge 健康文件回读为准。页面助手继续是只读建议任务；AI 设定整理是独立任务，只能回交待核建议。配置中的 AI 偏好、Bridge 启动或执行方式不会自行发起模型调用、授权付费生成，也不取消权利、版本、质量与来源门禁。数据库权限及宿主凭证由受信任维护者管理，业务事务不能代替操作系统隔离。
