...
本机 Codex Bridge 可以检查连接和读取冻结上下文。其自动启动、模型、并发和空闲回收时间在“系统配置 → AI与界面”按实例发布；配置生效以随后一次本地实例启动及 Bridge 健康文件回读为准。页面助手继续是只读建议任务；AI 设定整理是独立任务，只能回交待核建议。配置中的 AI 偏好、Bridge 启动或执行方式不会自行发起模型调用、授权付费生成，也不取消权利、版本、质量与来源门禁。数据库权限及宿主凭证由受信任维护者管理，业务事务不能代替操作系统隔离。
