...
新增本地 `writing` 命令组，保存逐片段候选、重读记录、正文与构想检查点，支持中断恢复；不自动调用模型，不增加过程页面，也不写正式审阅数据。仅完整小说通过全稿审校后，使用受控 `replace-source-content` 原地更新无评论、无依赖的故事精修条目；普通资料导入仍保持不可变语义。具体命令与限制见[后台逐步创作](docs/incremental-writing.md)。
